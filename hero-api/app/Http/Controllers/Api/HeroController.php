...
class HeroController extends Controller
{
    ...
    // 1. Tính giá nâng cấp (giữ nguyên)
    ...
    // 2. Trừ vàng và tăng level nâng cấp
    $player->gold -= $cost;
    $player->upgraded_attack_lv += 1;
    
    // 3. KHÔNG THAY ĐỔI BASE_ATTACK ở đây
    // $player->base_attack = (int)round($player->base_attack * 1.08); // XÓA DÒNG NÀY

    // 4. Lưu lại (Lúc này Model sẽ tự trigger sự kiện saving và cập nhật lại total_power)
    $player->save();

    return response()->json([
        'message' => 'Nâng cấp Attack thành công',
        'current_attack' => $player->current_attack, // Dùng accessor
