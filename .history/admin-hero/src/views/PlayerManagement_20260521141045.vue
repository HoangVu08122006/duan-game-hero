        ...
        <el-tabs type="border-card" class="detail-tabs">
          <el-tab-pane label="📊 Thông Số & Công Pháp">
            ...
            <div class="upgrade-grid">
              ...
              <div>💥 Cấp Sát Thương CM: <el-tag size="small" type="warning">Lv.{{ selectedPlayer.upgraded_crit_damage_lv || 1 }}</el-tag></div>
            </div>
          </el-tab-pane>
            ...
            <div v-if="selectedPlayer.pets && selectedPlayer.pets.length > 0">
              <div v-for="pet in selectedPlayer.pets" :key="pet.id" class="pet-card">
                ...
                <div class="pet-skills-list">
                  ...
                  <ul>
                    <li v-if="pet.skill_1_name">🔹 <b>{{ pet.skill_1_name }}</b> (Sát thương: <span class="power-text">{{ formatNumber(selectedPlayer.pet_skills?.skill_1_dame) }}</span>)</li>
                    <li v-if="pet.skill_2_name">🔹 <b>{{ pet.skill_2_name }}</b> (Sát thương: <span class="power-text">{{ formatNumber(selectedPlayer.pet_skills?.skill_2_dame) }}</span>)</li>
                    <li v-if="pet.skill_3_name">🔹 <b>{{ pet.skill_3_name }}</b> (Sát thương: <span class="power-text">{{ formatNumber(selectedPlayer.pet_skills?.skill_3_dame) }}</span>)</li>
                  </ul>
                </div>
              </div>
            </div>
          ...
          <el-tab-pane label="🔐 Tài Khoản & Bảo Mật">
            <div v-loading="accountLoading" class="account-settings-box">
              ...
              <el-form :model="accountForm" label-position="top" size="default">
                ...
                <el-form-item label="Mật Khẩu Mới (Bỏ trống nếu không muốn đổi)">
                  <el-input 
...
// 🛠️ THÊM MỚI: Các biến quản trị API Account
...
// 1. Lấy danh sách người chơi công khai
...
// 2. Gọi đồng thời API chi tiết, API tổng lực chiến & API Lấy thông tin Account tài khoản
const fetchPlayerDetail = async (playerId) => {
  ...
  try {
    const [detailRes, powerRes, accountRes] = await Promise.all([
      axios.get(`http://26.103.188.167:8000/api/list/players/${playerId}`),
      axios.get(`http://26.103.188.167:8000/api/list/players/${playerId}/total-power`),
      axios.get(`http://26.103.188.167:8000/api/list/players/${playerId}/account`) // API Get Account mới
    ])

    if (detailRes.data) {
      selectedPlayer.value = detailRes.data.player || detailRes.data.data || detailRes.data
    }
    ...
    // Gán dữ liệu Tên từ API Account trả về vào Form sửa đổi
    if (accountRes.data) {
      // Tùy theo cấu trúc BE trả về (ví dụ: accountRes.data.name hoặc accountRes.data.data.name)
      accountForm.name = accountRes.data.name || accountRes.data.player?.name || selectedPlayer.value?.name || ''
    }

  } catch (error) {
    ElMessage.error('Không thể lấy đầy đủ thông tin hoặc tài khoản người chơi!')
    console.error(error)
  } finally {
...
// 🛠️ THÊM MỚI: 2.1 Cập nhật tài khoản (PUT) bằng API mới cung cấp
const updatePlayerAccount = async () => {
  ...
  try {
    ...
    // Nếu admin nhập mật khẩu mới thì gửi đi, ngược lại không gửi để giữ nguyên pass cũ
    if (accountForm.password && accountForm.password.trim() !== '') {
      payload.password = accountForm.password
    }

    const response = await axios.put(`http://26.103.188.167:8000/api/list/players/${accountForm.id}/account`, payload)
    
    ElMessage.success('Cập nhật thông tin tài khoản Admin thành công!')
    
    // Đồng bộ nhanh tên mới ra ngoài danh sách cha và header popup
    ...
    // Clear trường password sau khi đổi thành công
    accountForm.password = ''
  } catch (error) {
    console.error(error)
    ElMessage.error(error.response?.data?.message || 'Có lỗi xảy ra khi sửa thông tin tài khoản!')
  } finally {
    accountUpdateLoading.value = false
  }
}

// 3. Gửi lệnh XÓA TÀI KHOẢN
...
.upgrade-grid { display: grid; grid-template-columns: repeat(auto-fit, minmax(130px, 1fr)); gap: 10px; background: #f4f4f5; padding: 12px; border-radius: 6px; font-size: 13px; }

/* Khối cấu hình Account tài khoản */
