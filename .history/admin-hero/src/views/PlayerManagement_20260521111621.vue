...
const fetchPlayerDetail = async (playerId) => {
  ...
  try {
    // CHÚ Ý: Đổi dấu hỏi chấm (?) thành dấu gạch chéo (/) ở đuôi link API
    const response = await axios.get(`http://26.103.188.167:8000/api/player/me/${playerId}`)
    
    if (response.data && response.data.player) {
      selectedPlayer.value = response.data.player
    } else {
      ElMessage.error('Dữ liệu API chi tiết sai cấu trúc mẫu.')
    }
  } catch (error) {
    console.error("Lỗi chi tiết:", error.response)
    ElMessage.error(`Lỗi API: ${error.response?.data?.message || 'Không thể tải chi tiết player!'}`)
  } finally {
