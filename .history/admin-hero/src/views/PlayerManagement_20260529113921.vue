<template>
  ...
  <el-descriptions :column="3" border size="small">
    ...
    <el-descriptions-item label="Cấp HP">💪 Lv. {{ selectedPlayer.upgraded_hp_lv }}</el-descriptions-item>
...
<h4 class="sub-section-title">⚙️ ĐIỀU CHỈNH CHỈ SỐ GỐC (GAME MASTER)</h4>
<div class="stats-edit-box">
  <el-form :model="statsBasicForm" inline size="small">
    <el-form-item label="💰 Vàng:"><el-input-number v-model="statsBasicForm.gold" :min="0" /></el-form-item>
    <el-form-item label="✨ EXP:"><el-input-number v-model="statsBasicForm.exp" :min="0" /></el-form-item>
    <el-form-item label="🆙 Cấp độ:"><el-input-number v-model="statsBasicForm.level" :min="1" /></el-form-item>
    <el-form-item label="🏰 Ải H.Tại:"><el-input-number v-model="statsBasicForm.current_floor" :min="0" /></el-form-item>
    <el-form-item label="🏆 Ải C.Nhất:"><el-input-number v-model="statsBasicForm.highest_floor" :min="0" /></el-form-item>
    
    <div style="margin-top: 10px;">
      <el-button type="warning" :loading="statsUpdateLoading" @click="updatePlayerBasicStats">
        💾 Lưu Thay Đổi Chỉ Số Gốc
      </el-button>
    </div>
  </el-form>
</div>
...
const updatePlayerStats = async () => {
  ...
  try {
    ...
    if (response.data.status === 'success') {
      ...
      // Gọi lại detail để hiển thị thông số mới nhất từ database
...
// 1. Khai báo state
const pData = detailRes.data.player; 
statsBasicForm.gold = pData.gold;
const statsBasicForm = reactive({
  gold: 0, exp: 0, level: 1, current_floor: 1, highest_floor: 1
});

// 2. Hàm lấy dữ liệu (được gọi trong fetchPlayerDetail)
const pData = detailRes.data.player; // Dữ liệu gốc từ DB
statsBasicForm.gold = pData.gold;
statsBasicForm.exp = pData.exp;
statsBasicForm.level = pData.level;
statsBasicForm.current_floor = pData.current_floor;
statsBasicForm.highest_floor = pData.highest_floor;

// 3. Hàm lưu thay đổi
const updatePlayerBasicStats = async () => {
  statsUpdateLoading.value = true;
  try {
    await axios.patch(`http://26.103.188.167:8000/api/list/players/${selectedPlayer.value.id}/stats`, statsBasicForm, { headers: getAuthHeaders() });
    ElMessage.success('Đã cập nhật chỉ số thành công!');
    await fetchPlayerDetail(selectedPlayer.value.id); // Lấy lại dữ liệu mới nhất
  } catch (error) {
