...
const fetchPlayerDetail = async (playerId) => {
  ...
  // 1. Lấy thông tin chính (Quan trọng nhất)
  try {
    const detailRes = await axios.get(`http://26.103.188.167:8000/api/list/players/${playerId}`, headers);
    ...
    // ... các gán khác của bạn ...
  } catch (e) {
    ElMessage.error('Lỗi khi lấy thông tin nhân vật');
  }

  // 2. Lấy Lực chiến (Không được để lỗi này chặn hiển thị thông tin nhân vật)
  try {
    const powerRes = await axios.get(`http://26.103.188.167:8000/api/list/players/${playerId}/total-power`, headers);
    if (powerRes.data.status === 'success') {
      powerDetails.value = powerRes.data;
    }
  } catch (e) {
    console.warn('Không lấy được lực chiến');
  }

  // 3. Lấy thông tin tài khoản
  try {
    const accountRes = await axios.get(`http://26.103.188.167:8000/api/list/players/${playerId}/account`, headers);
    accountForm.name = accountRes.data.data?.current_name || selectedPlayer.value?.name;
  } catch (e) {
    console.warn('Không lấy được thông tin tài khoản');
