          ...
          <div v-if="selectedPlayer.pets && selectedPlayer.pets.length > 0">
            <div v-for="pet in selectedPlayer.pets" :key="pet.id" class="pet-card">
              <div class="pet-info-main">
                ...
                <div class="pet-power">Lực chiến Pet: ⚡ {{ formatNumber(pet.calculated_stats?.total_power || pet.base_dps) }}</div>
              </div>
              
              <div class="pet-skills-list">
                <h5>🔥 Kỹ Năng Thần Thú:</h5>
                <ul>
                  <li>🔹 <b>{{ pet.skill_1_name }}</b> (Sát thương: 
                      <span class="power-text">{{ formatNumber(pet.calculated_stats?.skill_1_dame || 0) }}</span>)
                  </li>
                  <li>🔹 <b>{{ pet.skill_2_name }}</b> (Sát thương: 
                      <span class="power-text">{{ formatNumber(pet.calculated_stats?.skill_2_dame || 0) }}</span>)
                  </li>
                  <li>🔹 <b>{{ pet.skill_3_name }}</b> (Sát thương: 
                      <span class="power-text">{{ formatNumber(pet.calculated_stats?.skill_3_dame || 0) }}</span>)
                  </li>
                </ul>
              </div>
            </div>
...
const handleAdminUpgradePet = async (pet) => {
  try {
    ...
    // BƯỚC QUAN TRỌNG: Gán dữ liệu mới vào đúng đối tượng pet này
    pet.pivot.level = res.data.data.new_level;
    // Gán chỉ số mới vào thuộc tính riêng của pet
    pet.calculated_stats = Object.assign({}, res.data.data.new_stats);
...
const loadPlayerDetail = async (id) => {
  try {
    const res = await axios.get(`/api/list/players/${id}`);
    
    // Gán dữ liệu player mới
    const newPlayer = res.data.player;
    
    // Đảm bảo dữ liệu pet được cập nhật đúng cách
    selectedPlayer.value = {
        ...newPlayer,
        pets: newPlayer.playerPets.map(p => ({
            ...p,
            // Đảm bảo có thuộc tính calculated_stats từ backend
            calculated_stats: p.calculated_stats || {} 
        }))
    };
    
    console.log('Player đã load lại với chỉ số mới:', selectedPlayer.value);
  } catch (err) {
...
const loadPlayerDetail = async (id) => {
    const res = await axios.get(`/api/admin/player/${id}`);
    
    // Gán chỉ số cho từng pet ngay khi load xong
    selectedPlayer.value.pets = res.data.pets.map(p => ({
        ...p,
        calculated_stats: p.calculated_stats || calculateStatsLocal(p) 
    }));
};
