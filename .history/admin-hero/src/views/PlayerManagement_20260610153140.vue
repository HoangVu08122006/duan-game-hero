...
const handleAdminUpgradePet = async (pet) => {
    // 1. Lấy dữ liệu trực tiếp từ object 'pet' (không qua pivot vì log cho thấy không có pivot)
    const playerPetId = pet.pivot_id || pet.id; 
    const newLevel = parseInt(pet.edit_level);

    // 2. Kiểm tra dữ liệu trước khi gọi API
    if (!playerPetId || isNaN(newLevel) || newLevel < 1) {
        ElMessage.error('Dữ liệu không hợp lệ! Vui lòng nhập số cấp.');
        return;
    }
