...
// --- CẤU HÌNH CỐ ĐỊNH ---
// Thiết lập URL của Laravel API
const apiClient = axios.create({
  baseURL: 'http://localhost:8000', // Đổi port này nếu Laravel của bạn chạy port khác
...
const fetchData = async () => {
  try {
    // 1. Phải lấy CSRF cookie trước khi gọi API
    await apiClient.get('/sanctum/csrf-cookie');
    
    // 2. Sau đó mới lấy dữ liệu
    const { data } = await apiClient.get(`/api/admin/entity/${currentType.value}`);
    items.value = data.data;
  } catch (err) {
    console.error(err);
    alert('Lỗi: ' + (err.response?.status === 401 ? 'Bạn chưa đăng nhập!' : 'Không thể tải dữ liệu'));
  }
};
...
const saveDraft = async () => {
  try {
    // Gọi qua apiClient thay vì axios mặc định
