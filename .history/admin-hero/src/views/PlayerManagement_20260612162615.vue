...
<el-card shadow="never" class="table-card">
  ...
  <template #default="scope">
    ...
    <div v-if="!scope.row.is_online" style="font-size: 10px; color: #999; margin-top: 4px;">
...
const handleAdminUpgradePet = async (pet) => {
  try {
    ...
    const res = await axios.post(
      '/api/admin/pet/upgrade',
      payload
    )

    ElMessage.success(res.data.message)

    pet.pivot.level = pet.pivot.edit_level
  ...
  // Reload riêng, không ảnh hưởng thông báo thành công
  try {
...
const formatLastSeen = (dateString) => {
  ...
  if (diffInDays >= 1) return `${diffInDays} ngày trước`;
  
  const diffInHours = Math.floor((now - lastSeen) / (1000 * 60 * 60));
  if (diffInHours >= 1) return `${diffInHours} giờ trước`;
