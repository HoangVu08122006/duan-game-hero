...
const handleAdminSetLevel = async (row) => {
  try {
    // 1. Lấy token CSRF trước
    await axios.get('/sanctum/csrf-cookie'); 
    
    // 2. Sau đó mới thực hiện POST
    const res = await axios.post('/api/admin/weapon/set-level', {
      player_id: selectedPlayer.value.id,
      id: row.id, // Đảm bảo dùng ID chính xác
      level: row.tempLevel
    });
    
    ElMessage.success('Đã cập nhật cấp độ!');
    fetchPlayerDetail(selectedPlayer.value.id); 
  } catch (err) {
    console.error(err);
    ElMessage.error('Lỗi: ' + (err.response?.data?.message || 'Cập nhật thất bại!'));
  }
};
