  ...
  <div class="weapon-container" v-loading="loading">
    ...
    <el-dialog v-model="dialogVisible" :title="form.id ? 'Sửa Vũ Khí' : 'Thêm Vũ Khí'" width="400px">
      <el-form :model="form" label-position="top">
        ...
        <el-form-item label="Chọn ảnh từ máy">
...
const selectedFile = ref(null); // Lưu file được chọn
...
const saveWeapon = async () => {
  if (!form.value.name) return ElMessage.warning('Vui lòng nhập tên!');
  ...
  if (form.value.id) formData.append('_method', 'PUT'); // Để Laravel hiểu là PUT khi có file

  try {
    if (form.value.id) {
      await axios.post(`${API_URL}/${form.value.id}`, formData, { headers: {'Content-Type': 'multipart/form-data'} });
...
const confirmDelete = (id) => {
  ElMessageBox.confirm('Chắc chắn xóa?', 'Cảnh báo', { type: 'warning' })
    .then(async () => {
