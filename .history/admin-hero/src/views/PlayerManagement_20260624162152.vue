...
const viewPlayerDetail = async (playerId) => {
  try {
    const response = await axios.get(
      `http://26.103.188.167:8000/api/player/${playerId}`,
      { headers: getAuthHeaders() }
    )

    selectedPlayer.value = response.data.data

    console.log("CHI TIẾT PLAYER:", selectedPlayer.value)
    console.log("DANH SÁCH PET:", selectedPlayer.value.pets)
    console.log("PET SKILLS:", selectedPlayer.value.pet_skills)

  } catch (error) {
    ElMessage.error('Không thể lấy thông tin player!')
  }
}
