...
const handleAdminSetLevel = async (row) => {
  // Gỡ lỗi: Kiểm tra giá trị đang gửi lên
  console.log("Dữ liệu gửi đi:", {
    player_id: selectedPlayer.value.id,
    id: row.id, // Đảm bảo row.id là khóa chính trong bảng player_weapons
    level: row.tempLevel
  });

  try {
    const res = await axios.post(`http://26.103.188.167:8000/api/admin/weapon/set-level`, {
      player_id: selectedPlayer.value.id,
      id: row.id, // Sửa thành row.id nếu row.player_weapon_id là undefined
      level: row.tempLevel
    });
    ...
    // Hiển thị lỗi từ server để biết tại sao fail (rất quan trọng)
