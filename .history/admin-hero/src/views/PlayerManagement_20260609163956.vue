...
const handleAdminUpgradePet = async (pet) => {
  // Kiểm tra dữ liệu: pet và pet.pivot là bắt buộc
  if (!pet || !pet.pivot || !pet.pivot.id) {
    console.error('Đối tượng Pet không hợp lệ:', pet);
    ElMessage.error('Không tìm thấy ID của Pet!');
    return;
  }

  // Lấy cấp độ từ input, nếu input trống thì lấy level hiện tại
  const newLevel = parseInt(pet.pivot.edit_level || pet.pivot.level);
  
  if (isNaN(newLevel) || newLevel < 1) {
    ElMessage.warning('Vui lòng nhập cấp độ hợp lệ!');
    return;
  }

  petUpgradeLoading.value = pet.pivot.id;

  try {
    const response = await axios.post('http://26.103.188.167:8000/api/admin/pet/upgrade', {
      player_id: selectedPlayer.value.id,
      player_pet_id: pet.pivot.id, // Sử dụng ID chính xác ở đây
      new_level: newLevel
    }, { headers: getAuthHeaders() });
