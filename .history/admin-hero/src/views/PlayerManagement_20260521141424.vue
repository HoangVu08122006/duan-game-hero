...
// 2. Gọi đồng thời các API chi tiết tài khoản
const fetchPlayerDetail = async (playerId) => {
  ...
  try {
    const [detailRes, powerRes, accountRes] = await Promise.all([
      ...
      axios.get(`http://26.103.188.167:8000/api/list/players/${playerId}/account`)
    ])
    ...
    // Đúng cấu trúc map: `response.data.data.current_name` từ Laravel Controller trả về
    ...
    ElMessage.error('Không thể lấy đầy đủ thông tin tài khoản người chơi!')
    console.error(error)
  } finally {
...
const updatePlayerAccount = async () => {
  ...
  try {
    ...
    // Nếu điền mật khẩu mới và thỏa mãn tối thiểu 6 ký tự
    ...
    if (response.data && response.data.status === 'success') {
      ...
      // Đồng bộ tên mới ra Header của popup và danh sách bảng cha ngoài
    ...
    console.error(error)
    ElMessage.error(error.response?.data?.message || 'Cập nhật thất bại hoặc tên nhân vật đã tồn tại.')
  } finally {
    accountUpdateLoading.value = false
  }
}
