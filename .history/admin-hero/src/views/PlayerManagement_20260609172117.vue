...
const handleAdminUpgradePet = async (pet) => {
    // 1. Debug: Xem cấu trúc object pet trả về từ API
    console.log("Cấu trúc đầy đủ của pet:", pet);

    /**
     * Xác định playerPetId (ID của bản ghi trong bảng player_pets)
     * Ưu tiên pet.pivot.id vì đây là cấu trúc chuẩn của Laravel khi dùng withPivot
     */
    const playerPetId = pet.pivot?.id || pet.id; 

    /**
     * Xác định cấp độ mới
     * Ưu tiên edit_level (giá trị vừa nhập trong ô input), nếu chưa có thì lấy level hiện tại
     */
    const newLevel = pet.pivot?.edit_level || pet.pivot?.level || pet.level;

    console.log("Dữ liệu gửi lên API:", { playerPetId, newLevel });

    // 2. Validate dữ liệu trước khi gọi API
    if (!playerPetId) {
        ElMessage.error('Lỗi: Không tìm thấy ID bản ghi sở hữu (player_pet_id).');
        return;
    }
    
    if (newLevel === undefined || newLevel === null) {
        ElMessage.error('Lỗi: Cấp độ không hợp lệ!');
        return;
    }
    ...
    try {
        ...
        // 4. Thông báo thành công và làm mới dữ liệu
