    ...
    <el-descriptions-item label="Cấp Sát Thương CM">💥 Lv. {{ selectedPlayer.upgraded_crit_damage_lv }}</el-descriptions-item>
          ...
          <el-tab-pane label="⚔️ Vũ Khí Sở Hữu">
            <el-table :data="selectedPlayer.weapons" border size="small" style="width: 100%">
              ...
              <el-table-column label="Cấp Cường Hóa" width="220" align="center">
                <template #default="wScope">
                  <div style="display: flex; align-items: center; gap: 5px;">
                    <el-input-number 
                      v-model="wScope.row.tempLevel" 
                      size="small" 
                      :min="1" 
                      style="width: 80px;"
                    />
                    <el-button 
                      ...
                      size="small" 
                      plain 
                      @click="handleAdminSetLevel(wScope.row)"
                    >
                      Lưu
                    </el-button>
                  </div>
...
// Khi fetch dữ liệu người chơi về (ví dụ trong hàm fetchPlayerDetail)
const fetchPlayerDetail = async (id) => {
  const res = await axios.get(`/api/admin/players/${id}`);
  
  // Khởi tạo tempLevel cho từng vũ khí để Admin chỉnh sửa
  selectedPlayer.value.weapons = res.data.weapons.map(w => ({
    ...w,
    tempLevel: w.level // Gán cấp độ hiện tại vào biến tạm
  }));
};

// Hàm gửi cấp độ mới cho Admin
const handleAdminSetLevel = async (row) => {
  try {
    const res = await axios.post('/api/admin/weapon/set-level', {
      player_id: selectedPlayer.value.id,
      id: row.player_weapon_id, // ID trong bảng player_weapons
      level: row.tempLevel      // Cấp độ mới từ input
    });
    
    ElMessage.success('Đã cập nhật cấp độ!');
    fetchPlayerDetail(selectedPlayer.value.id); // Tải lại bảng để cập nhật dame
  } catch (err) {
