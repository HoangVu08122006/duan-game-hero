...
// 🛠️ THÊM: Biến lưu trữ thông điệp lỗi riêng cho trường Tên Nhân Vật
...
const fetchPlayerDetail = async (playerId) => {
  ...
  try {
    ...
    ElMessage.error('Không thể kết nối đến API chi tiết!')
  } finally {
...
const openRegisterDialog = () => { 
  nameFieldError.value = '' // Reset thông tin lỗi cũ khi mở form mới
...
const submitRegisterForm = async (formEl) => {
  ...
  await formEl.validate(async (valid) => {
    if (valid) {
      ...
      nameFieldError.value = '' // Đặt lại chuỗi trống trước khi gửi request
      ...
      try {
        ...
        // 🛠️ ĐÃ CẬP NHẬT: Kiểm tra nếu Backend báo lỗi trùng lặp (Mã lỗi thông thường là 400, 422 hoặc 409)
        const status = error.response ? error.response.status : null;
        const errMsg = error.response && error.response.data ? error.response.data.message : '';

        if (status === 400 || status === 422 || status === 409 || errMsg.includes('exist') || errMsg.includes('trùng')) {
          // Gán lỗi cụ thể vào Input để Element Plus tự động tô màu đỏ đường viền
