...
const handleAdminUpgradePet = async (pet) => {
    // Ưu tiên lấy ID từ pivot nếu có, nếu không lấy pet.id (tùy cấu trúc API trả về)
    // Dựa trên dữ liệu, bạn cần tìm trường id tương ứng với cột 'id' trong bảng 'player_pets'
    const playerPetId = pet.pivot?.id || pet.id; 
    
    // Đảm bảo lấy đúng level từ input
