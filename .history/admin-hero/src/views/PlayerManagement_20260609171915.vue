...
const handleAdminUpgrade = async (row) => {
  ...
  try {
    const response = await axios.post('http://26.103.188.167:8000/api/admin/weapon/upgrade', {
      player_id: selectedPlayer.value.id,
      weapon_id: row.id,
      level_to_add: row.upgrade_input
    }, { headers: getAuthHeaders() });
...
const handleAdminUpgradePet = async (pet) => {
  const playerPetId = pet.pivot ? pet.pivot.id : pet.id; 

    // Đảm bảo player_id là ID người chơi đang hiển thị trên modal
    const playerId = selectedPlayer.value.id; 

    console.log("Đang gửi:", { playerId, playerPetId });
    // Ưu tiên lấy giá trị từ input, nếu chưa nhập thì lấy giá trị hiện tại
    // 1. Log toàn bộ object pet để xem cấu trúc thực tế của nó trong Console
    console.log("Cấu trúc đầy đủ của pet:", pet);

    // 2. Tìm ID theo nhiều khả năng khác nhau
    // Thường nếu dùng Eloquent, nó nằm ở pet.pivot.id hoặc pet.id (nếu pet là record của bảng player_pets)
    const playerPetId = pet.pivot?.id || pet.id; 
    
    // Lấy level: ưu tiên giá trị người dùng đã sửa, sau đó đến level cũ
    const newLevel = pet.pivot?.edit_level || pet.pivot?.level || pet.level;

    console.log("Debug sau khi sửa:", { playerPetId, newLevel });

    if (!playerPetId) {
        ElMessage.error('Không tìm thấy ID thú cưng! Hãy kiểm tra dữ liệu trả về từ API.');
        return;
    }
    
    if (!newLevel) {
        ElMessage.error('Chưa có cấp độ để cập nhật!');
        return;
    }
   

    console.log("Debug Data:", { playerPetId, newLevel }); // Kiểm tra trong F12 Console

    if (!playerPetId || !newLevel) {
        ElMessage.error('Thiếu thông tin: Kiểm tra ID hoặc Level thú cưng!');
        return;
    }

    try {
        const response = await axios.post('http://26.103.188.167:8000/api/admin/pet/upgrade', {
            player_id: selectedPlayer.value.id,
        ...
        ElMessage.success(response.data.message);
        // Quan trọng: Gọi hàm fetch lại dữ liệu sau khi update thành công
        await fetchPlayerDetail(selectedPlayer.value.id);
    } catch (error) {
        ElMessage.error('Cập nhật thất bại: ' + (error.response?.data?.message || 'Lỗi server'));
    }
};
