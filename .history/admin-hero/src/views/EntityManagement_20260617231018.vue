...
// Cấu hình Axios để luôn đính kèm Token (tránh lỗi 401)
...
const deleteItem = async (id) => {
  if (confirm('Xác nhận xóa thực thể này?')) {
    await apiClient.delete(`/entity/${activeTab.value}/${id}`);
    fetchData();
  }
};

const editItem = (item) => alert('Mở form chỉnh sửa cho: ' + item.name);

// Tự động load lại khi đổi tab
