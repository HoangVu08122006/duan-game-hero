...
renderChart(labels, data) {
    const ctx = document.getElementById('playerChart').getContext('2d');
    new Chart(ctx, {
        // Sử dụng kiểu 'bar' làm base
        type: 'bar', 
        data: {
            labels: labels,
            datasets: [
                {
                    label: 'Số lượng tài khoản (Cột)',
                    data: data,
                    backgroundColor: '#42b883', // Màu cột
                    order: 2
                },
                {
                    label: 'Xu hướng (Đường)',
                    data: data,
                    type: 'line', // Ép kiểu line cho dataset này
                    borderColor: '#ff6384', // Màu đường
                    borderWidth: 3,
                    fill: false,
                    tension: 0.3, // Độ mượt của đường
                    order: 1
                }
            ]
        },
