...
// --- HÀM CHÍNH: Gọi API chi tiết của từng Player theo Route Param mới ---
const fetchPlayerDetail = async (playerId) => {
  ...
  try {
    // Đã sửa lại đúng URL: /api/list/players/{id}
    const response = await axios.get(`http://26.103.188.167:8000/api/list/players/${playerId}`)
    
    // Log ra để kiểm tra cấu trúc phản hồi của Laravel
    console.log("Dữ liệu chi tiết nhận được từ Backend:", response.data)

    if (response.data) {
      // Ép dữ liệu vào selectedPlayer dựa theo các trường có khả năng chứa object player
    ...
    // Đọc thông báo lỗi từ Laravel nếu có (ví dụ lỗi Chưa xác thực hoặc Sai ID)
