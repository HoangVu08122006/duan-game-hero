...
const handleAdminUpgradePet = async (pet) => {
    // THAY VÌ ĐOÁN, HÃY DÙNG CÁCH NÀY ĐỂ XEM ĐÚNG CÁI NÀO LÀ ID BẢNG player_pets
    console.log("DEBUG PET OBJECT:", pet); 

    // Thường nếu pet là một dòng từ bảng player_pets, ID của nó chính là pet.id
    // Hãy chắc chắn rằng 'pet' truyền vào hàm này là object đầy đủ
    const playerPetId = pet.id; 
    const newLevel = parseInt(pet.edit_level);

    if (!playerPetId) {
        ElMessage.error('Không tìm thấy ID thú cưng (player_pet_id)!');
        return;
    }

    try {
        pet.loading = true; // Bật loading trên card thú cưng

        // 3. Log để debug dữ liệu sẽ gửi đi (đã sửa lại cho khớp)
        console.log('Đang gửi dữ liệu nâng cấp:', {
            player_id: selectedPlayer.value.id,
            player_pet_id: playerPetId,
            new_level: newLevel
        });

        const response = await axios.post('/api/admin/pet/upgrade', {
            player_id: selectedPlayer.value.id,
            player_pet_id: playerPetId, 
            new_level: newLevel
        }, { headers: getAuthHeaders() });
        ...
        console.error('Lỗi khi nâng cấp Pet:', error);
        // Hiển thị thông báo lỗi chi tiết từ server nếu có
        ElMessage.error(error.response?.data?.message || 'Có lỗi xảy ra khi gọi API!');
    } finally {
        pet.loading = false; // Tắt loading dù thành công hay thất bại
    }
};
