...
const handleAdminUpgradePet = async (pet) => {
    // 1. Kiểm tra cấu trúc dữ liệu (đảm bảo pet có các thuộc tính cần thiết)
    // Nếu pet.pivot không tồn tại, hãy dùng chính pet hoặc kiểm tra lại tên biến
    const petId = pet.pivot?.id || pet.id; 
    const levelToSet = pet.pivot?.edit_level || pet.level;

    if (!petId) {
        ElMessage.error('Lỗi: Không tìm thấy ID thú cưng!');
        return;
    }

    try {
        // 2. Gửi dữ liệu đúng với tên mà Laravel yêu cầu
        const response = await axios.post('http://26.103.188.167:8000/api/admin/pet/upgrade', {
            ...
            player_pet_id: petId,               // Phải trùng với 'player_pet_id' trong validate
            new_level: parseInt(levelToSet)     // Phải trùng với 'new_level' trong validate
        }, { 
            headers: getAuthHeaders() // Đảm bảo header bao gồm Token xác thực
        });

        ElMessage.success(response.data.message);
        await fetchPlayerDetail(selectedPlayer.value.id);
    } catch (error) {
        // Log chi tiết lỗi để biết tại sao Laravel từ chối
