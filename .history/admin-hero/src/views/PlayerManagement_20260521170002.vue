        ...
        <el-tabs type="border-card" class="detail-tabs">
          <el-tab-pane label="📊 Thông Số & Công Pháp">
            <el-descriptions :column="2" border size="small">
              ...
              <el-descriptions-item label="Máu hiện tại (HP)">❤️ {{ selectedPlayer.base_hp }}</el-descriptions-item>
...
const updatePlayerStats = async () => {
  ...
  try {
    ...
    if (response.data && response.data.status === 'success') {
      ...
      // 1. Cập nhật trực tiếp dữ liệu nhân vật từ response của API
      // Giả sử API trả về 'new_stats' như đoạn code PHP bạn đã gửi lúc trước
      if (selectedPlayer.value && response.data.new_stats) {
        // Cập nhật các chỉ số gốc mới nhất
        selectedPlayer.value.base_attack = response.data.new_stats.attack_lv || selectedPlayer.value.base_attack; 
        // Lưu ý: Hãy kiểm tra chính xác tên biến mà Backend trả về ở new_stats
        
        // Cập nhật lại các cấp độ trong UI
      ...
      // 2. Gọi lại API lực chiến để làm mới 🔥
      ...
      if (powerRes.data && powerRes.data.status === 'success') {
        ...
        // Cập nhật luôn vào selectedPlayer để hiển thị đúng
        selectedPlayer.value.total_power = powerRes.data.calculated_total_power
      }
    }
  } catch (error) {
    console.error(error)
    ElMessage.error(error.response?.data?.message || 'Lỗi hệ thống!')
  } finally {
