...
const handleAdminUpgradePet = async (pet) => {
  // Lấy ID từ bảng player_pets. Dựa trên log của bạn, hãy xem object pet 
  // có thuộc tính nào là id của bảng trung gian không (thường là pet.pivot.id)
  const playerPetId = pet.pivot ? pet.pivot.id : null; 
  ...
  try {
    const response = await axios.post('http://26.103.188.167:8000/api/admin/pet/upgrade', {
      player_id: selectedPlayer.value.id,
      player_pet_id: playerPetId, // Gửi ID đúng (ví dụ: 2, 4, 7, 8...)
      new_level: newLevel
    }, { headers: getAuthHeaders() });
