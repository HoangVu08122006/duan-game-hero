          ...
          <div v-if="selectedPlayer.pets && selectedPlayer.pets.length > 0">
            <div v-for="pet in selectedPlayer.pets" :key="pet.id" class="pet-card">
              <div class="pet-info-main">
                ...
                <div class="pet-power">Lực chiến Pet: ⚡ {{ formatNumber(pet.calculated_stats?.total_power || 0) }}</div>
              </div>
              
              <div class="pet-skills-list">
                <h5>🔥 Kỹ Năng Thần Thú:</h5>
                <ul>
                  <li>🔹 <b>{{ pet.skill_1_name }}</b> (Sát thương: <span class="power-text">{{ formatNumber(pet.calculated_stats?.skill_1_dame || 0) }}</span>)</li>
                  <li>🔹 <b>{{ pet.skill_2_name }}</b> (Sát thương: <span class="power-text">{{ formatNumber(pet.calculated_stats?.skill_2_dame || 0) }}</span>)</li>
                  <li>🔹 <b>{{ pet.skill_3_name }}</b> (Sát thương: <span class="power-text">{{ formatNumber(pet.calculated_stats?.skill_3_dame || 0) }}</span>)</li>
                </ul>
              </div>
            </div>
          </div>
  ...
  try {
    ...
    selectedPlayer.value = {
        ...
        pets: newPlayer.playerPets.map(p => ({
            ...p,
            // KHỞI TẠO ĐỂ ĐẢM BẢO REACTIVITY
            pivot: {
                ...p.pivot,
                edit_level: p.pivot.level // Gán giá trị mặc định cho input
            },
            calculated_stats: p.calculated_stats || { 
                total_power: 0, skill_1_dame: 0, skill_2_dame: 0, skill_3_dame: 0 
            }
        }))
    };
  } catch (err) {
...
const handleAdminUpgradePet = async (pet) => {
  try {
    ...
    // CẬP NHẬT TRỰC TIẾP TRÊN ĐỐI TƯỢNG PET ĐƯỢC CHỌN
    pet.pivot.level = res.data.data.new_level;
    // Gán mới object stats để Vue nhận diện thay đổi
