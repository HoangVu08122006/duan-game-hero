...
const handleAdminUpgradePet = async (pet) => {
    // 1. Xác định ID chính xác của bản ghi trong bảng 'player_pets'
    // Dựa trên database của bạn, đây là khóa chính duy nhất để xác định pet của từng người
    const playerPetId = pet.pivot?.id || pet.id; 

    // 2. Xác định Level mới (ưu tiên lấy từ input của admin, nếu không có lấy level hiện tại)
    const newLevel = pet.edit_level || pet.level;

    // 3. Debug kiểm tra trước khi gửi
    console.log("Debug gửi lên API:", { 
        player_id: selectedPlayer.value.id, 
        player_pet_id: playerPetId, 
        new_level: newLevel 
    });

    // 4. Validate: Nếu thiếu thông tin hoặc ID vẫn là 1 (nghi vấn sai ID), chặn ngay
    ...
    try {
        ...
        const response = await axios.post('http://26.103.188.167:8000/api/admin/pet/upgrade', {
            player_id: selectedPlayer.value.id, // ID người chơi
            player_pet_id: playerPetId,         // ID bản ghi bảng player_pets (ví dụ: 2, 4, 7...)
            new_level: parseInt(newLevel)       // Đảm bảo là kiểu số nguyên
        }, { headers: getAuthHeaders() });

        // 6. Xử lý sau khi thành công
        ElMessage.success(response.data.message || 'Cập nhật Pet thành công!');
        
        // Làm mới dữ liệu người chơi để cập nhật giao diện
        if (typeof fetchPlayerDetail === 'function') {
            await fetchPlayerDetail(selectedPlayer.value.id);
        }
    } catch (error) {
        // Hiển thị lỗi chi tiết từ server
        const errorMsg = error.response?.data?.message || 'Có lỗi xảy ra, không thể cập nhật!';
        ElMessage.error('Cập nhật thất bại: ' + errorMsg);
