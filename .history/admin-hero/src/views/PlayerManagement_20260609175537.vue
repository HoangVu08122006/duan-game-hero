...
const handleAdminUpgradePet = async (pet) => {
    // 1. Lấy ID từ pivot (đây là ID bảng player_pets)
    const playerPetId = pet.pivot?.id; 

    // 2. Lấy giá trị từ input (đã được khởi tạo ở bước 1)
    const newLevel = pet.pivot?.edit_level;

    console.log("DEBUG ID và Level:", { 
        player_id: selectedPlayer.value.id, 
...
const fetchPlayerDetail = async (playerId) => {
  ...
  try {
    ...
    // Khởi tạo edit_level cho mỗi pet để v-model có thể bind được
    selectedPlayer.value = res.data.player;
    selectedPlayer.value.pets.forEach(pet => {
        // Gán giá trị mặc định bằng cấp hiện tại nếu chưa có
        pet.pivot.edit_level = pet.pivot.level; 
    });
