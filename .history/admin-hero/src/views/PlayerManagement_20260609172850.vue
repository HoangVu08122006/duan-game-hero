...
const handleAdminUpgradePet = async (pet) => {
    // 1. Lấy đúng ID (như đã xác định là pet.id vì đây là record bảng player_pets)
    const playerPetId = pet.id; 

    // 2. ÉP BUỘC lấy giá trị từ input người dùng
    // Nếu pet.pivot.edit_level không tồn tại, lấy pet.level làm mặc định
    const newLevel = pet.pivot?.edit_level !== undefined ? pet.pivot.edit_level : pet.level;

    console.log("--- DEBUG UPDATE ---");
    console.log("ID Pet (bảng player_pets):", playerPetId);
    console.log("Cấp cũ trong DB:", pet.level);
    console.log("Cấp mới từ Input (newLevel):", newLevel); // PHẢI LÀ 1

    if (!playerPetId || !newLevel) {
        ElMessage.error('Thiếu thông tin!');
        return;
    }
    ...
    try {
        const response = await axios.post('http://26.103.188.167:8000/api/admin/pet/upgrade', {
            ...
            new_level: parseInt(newLevel) // Đảm bảo chuyển sang số nguyên
        }, { headers: getAuthHeaders() });

        ElMessage.success(response.data.message);
        await fetchPlayerDetail(selectedPlayer.value.id); // Refresh lại để thấy cấp mới
    } catch (error) {
