         ...
         <el-tab-pane label="🐉 Trận Doanh Thú Cưng">
          <div v-if="selectedPlayer.pets && selectedPlayer.pets.length > 0">
            <div v-for="pet in selectedPlayer.pets" :key="pet.id" class="pet-card">
              ...
              <div class="pet-skills-list" v-if="pet.stats">
                <h5>🔥 Kỹ Năng Thần Thú:</h5>
                <ul>
                  <li>🔹 <b>Skill 1</b> (Sát thương: {{ formatNumber(pet.stats.skill_1_dame) }})</li>
                  <li>🔹 <b>Skill 2</b> (Sát thương: {{ formatNumber(pet.stats.skill_2_dame) }})</li>
                  <li>🔹 <b>Skill 3</b> (Sát thương: {{ formatNumber(pet.stats.skill_3_dame) }})</li>
                </ul>
              </div>
            </div>
          </div>
...
const handleAdminUpgradePet = async (pet) => {
  try {
    ...
    // Cập nhật level và chỉ số cho đúng con pet đang thao tác
    pet.level = res.data.data.new_level;
    pet.stats = res.data.data.new_stats; 
