...
const handleAdminUpgradePet = async (pet) => {
    // Kiểm tra log console để xem đối tượng 'pet' thực sự chứa gì
    console.log("Đối tượng pet truyền vào:", pet);

    // Dựa trên bảng của bạn, bản ghi sở hữu cần 'id' từ bảng player_pets
    // Thường nếu dùng quan hệ, nó sẽ nằm trong pet.pivot.id
    const playerPetId = pet.pivot ? pet.pivot.id : pet.id;
    
    // Đảm bảo lấy được level từ input (v-model="pet.pivot.edit_level")
    const newLevel = pet.pivot ? pet.pivot.edit_level : pet.edit_level;
    ...
    try {
        const response = await axios.post('http://26.103.188.167:8000/api/admin/pet/upgrade', {
            player_id: selectedPlayer.value.id,
            player_pet_id: playerPetId, // Gửi ID của bản ghi sở hữu
