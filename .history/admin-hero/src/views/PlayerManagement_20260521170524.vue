...
const updatePlayerStats = async () => {
  ...
  try {
    ...
    if (response.data && response.data.status === 'success') {
      ...
      // QUAN TRỌNG: Cập nhật lại đối tượng selectedPlayer từ response
      if (selectedPlayer.value && response.data.new_stats) {
          selectedPlayer.value.base_attack = response.data.new_stats.base_attack;
          selectedPlayer.value.base_hp = response.data.new_stats.base_hp;
          selectedPlayer.value.total_power = response.data.new_stats.total_power;
          
          // Cập nhật cả cấp độ nếu cần
