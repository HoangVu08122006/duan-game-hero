...
const handleAdminUpgradePet = async (pet) => {
    ...
    // 2. Lấy giá trị từ edit_level (gán mặc định là level hiện tại nếu chưa nhập gì)
    const newLevel = pet.pivot?.edit_level || pet.pivot?.level;
    ...
    try {
        const response = await axios.post('http://26.103.188.167:8000/api/admin/pet/upgrade', {
            player_id: selectedPlayer.value.id, // Đảm bảo selectedPlayer đã được load đúng
            player_pet_id: playerPetId, 
            new_level: parseInt(newLevel)
        }, { headers: getAuthHeaders() });

        ElMessage.success(response.data.message);
        // Sau khi thành công, refresh lại dữ liệu để cập nhật giao diện
        await fetchPlayerDetail(selectedPlayer.value.id);
    } catch (error) {
