...
const handleEquipWeapon = async (weapon) => {
  equipLoading.value = weapon.id; // Bật loading cho đúng dòng đó
  try {
    // Gọi API trang bị (Admin thao tác cho người chơi)
    const response = await axios.post(`http://26.103.188.167:8000/api/admin/weapon/equip`, {
      player_id: selectedPlayer.value.id, // ID người chơi đang xem
      id: weapon.id // ID bản ghi trong player_weapons
    }, { headers: getAuthHeaders() });

    if (response.data.status === 'success') {
      ElMessage.success('Đã trang bị vũ khí thành công!');
      // Tải lại thông tin người chơi để cập nhật trạng thái bảng
      await fetchPlayerDetail(selectedPlayer.value.id);
    }
  } catch (error) {
    ElMessage.error(error.response?.data?.message || 'Có lỗi xảy ra!');
  } finally {
    equipLoading.value = null; // Tắt loading
  }
};
