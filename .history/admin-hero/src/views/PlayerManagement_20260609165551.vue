...
const handleAdminUpgradePet = async (pet) => {
  // 1. Kiểm tra log thực tế xem 'pet' chứa gì
  console.log("Đối tượng pet nhận được:", pet);

  // 2. Tìm ID bản ghi từ bảng player_pets. 
  // Nếu dữ liệu trả về từ API là object PlayerPet, id của nó chính là pet.id
  // Nếu dữ liệu là quan hệ, id thường nằm ở pet.id hoặc pet.pivot.id
  const playerPetId = pet.id || pet.pivot?.id;

  if (!playerPetId) {
    ElMessage.error('Không tìm thấy ID bản ghi! Kiểm tra log Console.');
    return;
  }
  ...
  const payload = {
    ...
    new_level: parseInt(pet.edit_level || pet.level)
  };

  try {
    const response = await axios.post('http://26.103.188.167:8000/api/admin/pet/upgrade', payload, { headers: getAuthHeaders() });
    ElMessage.success(response.data.message);
    await fetchPlayerDetail(selectedPlayer.value.id);
  } catch (error) {
