              ...
              <div class="pet-info-main">
                ...
                <div class="admin-control" style="margin-top: 10px; padding: 10px; border: 1px dashed #e6a23c; border-radius: 4px;">
                  ...
                  <el-button 
                    type="warning" 
                    size="small" 
                    @click="handleAdminUpgradePet(pet.pivot)"
                  >
                    Áp dụng
                  </el-button>
                </div>
...
Để đoạn code của bạn hoàn chỉnh và loại bỏ dứt điểm lỗi 422 (The player_pet_id field is required), bạn cần đảm bảo pivot.id thực sự tồn tại khi truyền vào hàm.

Dưới đây là phiên bản "chống lỗi" cao cấp:

1. Cập nhật script (Vue.js)
Tôi đã thêm kiểm tra pivot và logic để lấy id an toàn hơn.

JavaScript
const petUpgradeLoading = ref(null);

const handleAdminUpgradePet = async (pivot) => {
  // Kiểm tra pivot có tồn tại không
  if (!pivot || !pivot.id) {
    ElMessage.error('Không tìm thấy thông tin Pet để cập nhật!');
    return;
  }

  // 1. Kiểm tra giá trị level
  const newLevel = parseInt(pivot.edit_level); 
  if (isNaN(newLevel) || newLevel < 1) {
    ElMessage.warning('Vui lòng nhập cấp độ hợp lệ (từ 1 trở lên)!');
    return;
  }

  // 2. Bắt đầu loading
  petUpgradeLoading.value = pivot.id;

  try {
    const payload = {
      player_id: selectedPlayer.value.id,
      player_pet_id: pivot.id, // Đảm bảo đây chính xác là ID của record trong bảng trung gian
      new_level: newLevel
    };

    console.log('Đang gửi dữ liệu:', payload); // DEBUG: Kiểm tra trong F12

    const response = await axios.post('http://26.103.188.167:8000/api/admin/pet/upgrade', payload, { 
      headers: getAuthHeaders() 
    });
    ...
    if (error.response?.status === 422) {
      ...
      // Hiển thị lỗi dễ đọc hơn cho Admin
      const msg = Object.values(error.response.data.errors).flat().join(', ');
      ElMessage.error('Lỗi dữ liệu: ' + msg);
    } else {
      ElMessage.error('Cập nhật thất bại: ' + (error.response?.data?.message || 'Lỗi hệ thống'));
    }
  } finally {
    petUpgradeLoading.value = null;
  }
};
