...
const handleAdminUpgradePet = async (pet) => {
    ...
    // pivot.id là chìa khóa để phân biệt thú cưng của người chơi này với người chơi khác
    const playerPetId = pet.pivot?.id || pet.id; 

    // 2. Xác định Level mới từ input của admin
    const newLevel = pet.edit_level || pet.level;

    // 3. Debug kiểm tra trước khi gửi
    console.log("DEBUG ID - Đang gửi:", { 
        player_id: selectedPlayer.value.id, 
        player_pet_id: playerPetId, 
        new_level: newLevel 
    });

    // 4. Validate dữ liệu chặt chẽ
    if (!playerPetId) {
        ElMessage.error('Lỗi: Không tìm thấy ID bản ghi (player_pet_id)!');
        return;
    }
    ...
    try {
        ...
        // Làm mới dữ liệu để cập nhật giao diện ngay lập tức
