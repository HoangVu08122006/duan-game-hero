      ...
      <div v-else-if="selectedPlayer" class="detail-wrapper">
        ...
        <el-tabs type="border-card" class="detail-tabs">
          <el-tab-pane label="📊 Thông Số & Công Pháp">
            ...
            <h4 class="sub-section-title">✨ Cấp Độ Thuộc Tính</h4>
...
// 2. Gọi đồng thời API chi tiết & API tổng lực chiến mới
const fetchPlayerDetail = async (playerId) => {
  ...
  try {
    const [detailRes, powerRes] = await Promise.all([
    ...
    if (powerRes.data && powerRes.data.status === 'success') {
      ...
      // 🛠️ ĐÃ THÊM: Cập nhật đồng bộ ngược lại cột lực chiến ngoài bảng chính cho tài khoản vừa xem
    ...
    ElMessage.error('Không thể lấy dữ liệu chi tiết và lực chiến người chơi!')
    console.error(error)
  } finally {
