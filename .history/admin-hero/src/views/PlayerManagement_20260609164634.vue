...
const handleAdminUpgradePet = async (playerPet) => {
  // playerPet.id ở đây chính là id của bảng player_pets
  if (!playerPet || !playerPet.id) {
    ElMessage.error('Không tìm thấy ID của bản ghi thú cưng!');
    return;
  }

  const newLevel = parseInt(playerPet.edit_level || playerPet.level);

  try {
    const response = await axios.post('http://26.103.188.167:8000/api/admin/pet/upgrade', {
      player_id: selectedPlayer.value.id,
      player_pet_id: playerPet.id, // ID này chắc chắn hợp lệ vì nó lấy từ bảng player_pets
      new_level: newLevel
    }, { headers: getAuthHeaders() });

    ElMessage.success(response.data.message);
    await fetchPlayerDetail(selectedPlayer.value.id); // Refresh data
} catch (error) {
  if (error.response && error.response.status === 422) {
    console.table(error.response.data.errors); // In ra bảng lỗi rất dễ nhìn
    ElMessage.error('Lỗi dữ liệu: ' + JSON.stringify(error.response.data.errors));
  } else {
    ElMessage.error('Lỗi: ' + (error.response?.data?.message || 'Không xác định'));
  }
}
};
