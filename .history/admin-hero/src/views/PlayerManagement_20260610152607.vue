...
console.log('Thông tin con thú:', pet); // Xem con thú này có chứa thông tin gì
    console.log('Pivot của con thú:', pet.pivot);
    const playerPetId = pet.id; 
    const newLevel = parseInt(pet.edit_level); // Lưu ý: Dùng pet.edit_level thay vì pet.pivot.edit_level

    if (!playerPetId || isNaN(newLevel) || newLevel < 1) {
        ElMessage.error('Dữ liệu không hợp lệ!');
        return;
    }

    try {
        pet.loading = true; // Bật loading
        // Trong hàm handleAdminUpgradePet(pet)
          console.log('Đang gửi dữ liệu:', {
              player_id: selectedPlayer.value.id,
              player_pet_id: pet.pivot.id, // Kiểm tra xem pet.pivot.id có tồn tại không
        ...
        await fetchPlayerDetail(selectedPlayer.value.id); // Tải lại data
    } catch (error) {
        console.error(error);
        ElMessage.error(error.response?.data?.message || 'Có lỗi xảy ra!');
    } finally {
        pet.loading = false; // Tắt loading
    }
};
