...
const handleAdminUpgradePet = async (pet) => {
  // 1. Kiểm tra input: Sử dụng pet.edit_level tương ứng với row.upgrade_input
  if (!pet.edit_level || pet.edit_level < 1) {
    ElMessage.warning('Vui lòng nhập số cấp hợp lệ!');
    return;
  }

  try {
    pet.loading = true; // Bật loading trên card

    // 2. Gửi dữ liệu: Dùng pet.id làm player_pet_id
    // Nếu pet.id này vẫn báo lỗi, hãy thay bằng: pet.pivot_id hoặc pet.player_pet_id
    const response = await axios.post('/api/admin/pet/upgrade', {
      player_id: selectedPlayer.value.id,
      player_pet_id: pet.id, // ID của bản ghi trong bảng player_pets
      new_level: parseInt(pet.edit_level)
    }, { headers: getAuthHeaders() });

    ElMessage.success(response.data.message || 'Cập nhật thành công!');

    // 3. Cập nhật lại dữ liệu sau khi nâng cấp
    await fetchPlayerDetail(selectedPlayer.value.id);
    
    // Nếu có log, gọi thêm loadLogs giống hàm vũ khí
    if (typeof loadLogs === 'function') {
      await loadLogs(selectedPlayer.value.id);
    }

  } catch (error) {
    console.error('Nâng cấp Pet thất bại:', error);
    ElMessage.error('Nâng cấp thất bại: ' + (error.response?.data?.message || 'Lỗi server'));
  } finally {
