...
// --- BIẾN ĐIỀU KHIỂN XEM CHI TIẾT (ME API) ---
...
// 1. Lấy danh sách thô từ Backend đổ ra bảng
...
// 2. HÀM CHÍNH: Gọi API chi tiết của từng Player cụ thể bằng ID
const fetchPlayerDetail = async (playerId) => {
  ...
  try {
    // Thử gửi dạng param chuẩn ?id=
    const response = await axios.get(`http://26.103.188.167:8000/api/list/players?id=${playerId}`)
    
    // In hẳn ra tab Console (F12) để bạn nhìn xem server trả về chữ gì
    console.log("Dữ liệu thực tế từ API trả về:", response.data)

    // Kiểm tra nếu object trả về chứa thông tin player trực tiếp hoặc bọc trong .player
    if (response.data) {
      // Nếu có response.data.player thì lấy, không thì lấy luôn response.data hoặc response.data.data
      selectedPlayer.value = response.data.player || response.data.data || response.data
    } else {
      ElMessage.error('API trả về rỗng')
    }
  } catch (error) {
    console.error("Lỗi chi tiết từ server:", error.response)
    ElMessage.error(`Lỗi API: ${error.response?.data?.message || 'Không tìm thấy route hoặc server lỗi!'}`)
  } finally {
    detailLoading.value = false
  }
}

// 3. Xử lý Form Đăng ký tài khoản mới
...
const handleKick = (row) => { ElMessage.error(`Đã kích hoạt Kick: ${row.name}`) }
