...
const handleAdminUpgradePet = async (pet) => {
  ...
  const newLevel = parseInt(pet.edit_level || pet.level);
  ...
  // DEBUG: Kiểm tra kỹ payload trước khi gửi
  ...
  try {
    const response = await axios.post('http://26.103.188.167:8000/api/admin/pet/upgrade', payload, { 
        headers: { 
            ...getAuthHeaders(),
            'Content-Type': 'application/json' // Đảm bảo header chuẩn
        } 
    });
    ElMessage.success(response.data.message);
    await fetchPlayerDetail(selectedPlayer.value.id);
  } catch (error) {
    // In ra toàn bộ thông báo lỗi từ Laravel
    console.error("Lỗi từ server:", error.response?.data);
    ElMessage.error('Lỗi: ' + JSON.stringify(error.response?.data?.errors || error.response?.data?.message));
  }
};
