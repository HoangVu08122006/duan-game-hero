...
const handleAdminUpgradePet = async (pet) => {
  // Gỡ lỗi: Xem cấu trúc pet thực tế
  console.log('Dữ liệu pet truyền vào:', pet);
  
  // Nếu pet.pivot.id không có, hãy kiểm tra xem nó có nằm trực tiếp ở pet.id không
  // (Trong bảng player_pets, cột id là khóa chính)
  const playerPetId = pet.pivot ? pet.pivot.id : pet.id; 

  if (!playerPetId) {
    ElMessage.error('Không tìm thấy ID bản ghi (player_pet_id). Kiểm tra lại backend!');
    return;
  }
  ...
  try {
    const response = await axios.post('http://26.103.188.167:8000/api/admin/pet/upgrade', {
      ...
      new_level: parseInt(pet.pivot?.edit_level || pet.pivot?.level)
    }, { headers: getAuthHeaders() });

    ElMessage.success(response.data.message);
    await fetchPlayerDetail(selectedPlayer.value.id);
  } catch (error) {
    // Hiển thị lỗi từ Laravel để biết chính xác nó đang kêu thiếu field nào
