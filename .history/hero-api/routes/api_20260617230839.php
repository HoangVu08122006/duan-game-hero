...
// Đảm bảo bạn đã có middleware kiểm tra quyền admin (ví dụ: 'auth:admin')
Route::middleware(['auth:admin'])->prefix('admin')->group(function () {

    // Route quản lý thực thể (Monster/Boss)
    // {type} sẽ nhận giá trị là 'monster' hoặc 'boss'
    Route::prefix('entity/{type}')->group(function () {
        Route::get('/', [AdminEntityController::class, 'index']);      // Danh sách
        Route::post('/draft', [AdminEntityController::class, 'storeDraft']); // Lưu nháp
        Route::delete('/{id}', [AdminEntityController::class, 'destroy']);   // Xóa
    });

    // Route duyệt bản nháp (không cần {type} vì đã có {draftId})
    Route::post('entity/publish/{draftId}', [AdminEntityController::class, 'publishDraft']);
});

Route::middleware(['auth:admin'])->prefix('admin')->group(function () {
    // API hiển thị danh sách quái
    Route::get('/monsters', [AdminEntityController::class, 'indexMonster']);
    
    // API hiển thị danh sách boss
    Route::get('/bosses', [AdminEntityController::class, 'indexBoss']);
});