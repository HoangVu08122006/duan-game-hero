...
class Player extends Authenticatable { // Đổi từ Model thành Authenticatable
    ...
    // Chỉ số cơ bản & Tài nguyên
    'gold', 'level', 'exp', 'kill_count',
    'base_hp', 'current_hp', 
    'base_attack', 'current_attack',
    'base_crit_rate', 'base_crit_damage', 'base_speed',

    // Tiến trình leo tầng
    'current_floor', 'highest_floor', 'reached_highest_floor_at',
...
protected static function booted()
{
    ...
    // CƠ CHẾ 0: Đảm bảo dữ liệu mặc định luôn được thiết lập
    static::creating(function ($player) {
        if (is_null($player->current_hp)) {
            $player->current_hp = $player->base_hp;
        }
    });

    // CƠ CHẾ 1: Tặng vũ khí mặc định
    ...
    // CƠ CHẾ 2: Tính Level & EXP
    static::saving(function ($player) {
        while (true) {
            $currentLevel = $player->level;
            $expNeeded = ($currentLevel < 10) ? (100 + ($currentLevel - 1) * 50) : 
                         (($currentLevel < 50) ? (int)round(500 * pow(1.1, $currentLevel - 9)) : 
                         (int)round((int)round(500 * pow(1.1, 40)) * pow(1.05, $currentLevel - 49)));
...
public function playerPets() {
    return $this->hasMany(PlayerPet::class);
}
...
public function getCurrentAttackAttribute()
{
    // Kiểm tra xem $this->base_attack có dữ liệu chưa?
    $multiplier = pow(1.08, $this->upgraded_attack_lv);
    $result = (int)round($this->base_attack * $multiplier);
    
    // Debug: Thử trả về 1 con số cụ thể xem nó có hiện ra JSON không
...
/**
    ...
    /**
     * CƠ CHẾ CHIẾN ĐẤU: Tính tổng lực công thực tế (Chỉ cộng dồn 1 vũ khí + 1 pet đang trang bị)
     */
    public function getTotalDamage()
    {
        // 1. Sát thương thực tế từ bản thân Hero (Đã bao gồm % cấp nâng cấp cường hóa)
        $heroDame = $this->current_attack; // Gọi trực tiếp Accessor getCurrentAttackAttribute() ở trên

        // 2. Sát thương từ Vũ khí đang trang bị
        $weaponDame = 0;
        ...
        // 3. Sát thương từ DUY NHẤT chú Pet đang trang bị (Bao gồm cả kỹ năng kích hoạt)
        $petDame = 0;
        $petSkillsData = $this->pet_skills; // Gọi trực tiếp khối logic vừa viết phía trên thông qua Appends Attribute
        
        if (isset($petSkillsData['total_power'])) {
            $petDame = $petSkillsData['total_power'];
        }

        // Trả về tổng damage không bị cộng dồn từ các pet nằm trong kho đồ
        return (int)($heroDame + $weaponDame + $petDame);
    }

    // --- BỔ SUNG CÁC ACCESOR ĐỂ SỬA LỖI ---

public function getCurrentCritRateAttribute()
{
    // Công thức: gốc + (cấp * 0.1)
    return (float)($this->base_crit_rate + ($this->upgraded_crit_rate_lv * 0.1));
}

public function getCurrentCritDamageAttribute()
{
    // Công thức: gốc + (cấp * 0.5)
    return (float)($this->base_crit_damage + ($this->upgraded_crit_damage_lv * 0.5));
}

public function getCurrentSpeedAttribute()
{
    // Công thức: gốc + (cấp * 0.2)
    return (float)($this->base_speed + ($this->upgraded_speed_lv * 0.2));
}

// App\Models\Player.php

public function getHeroPowerAttribute() {
    $stats = $this->calculatedStatsAttribute; // Lấy từ Accessor ở bước trước
    
    // Công thức tính Lực chiến bản thân (Hero Base)
    // Bao gồm: Attack + (HP/10) + (CritRate*5000) + (CritDamage*2000) + (Speed*100)
    $power = $stats['atk'] 
             + ($stats['hp'] / 10) 
             + (($stats['crit_r']) * 5000) 
             + (($stats['crit_d'] - 1) * 2000) 
...
{
    // Nếu last_login_at cũ hơn 5 phút (300 giây), coi như Offline
    if ($this->last_login_at && now()->diffInSeconds($this->last_login_at) > 300) {
        return 'Offline';
    }
