...
public function getTotalDamage()
    {
        $heroDame = $this->current_attack;
        
        $weaponDame = $this->weapons()->where('is_equipped', true)->sum('current_damage') ?? 0;
    ...
    // HÀM 2: Tính lực chiến tổng (Gọi hàm 1 + tính các chỉ số phụ)
    public function calculateTotalPower() 
    {
        // Lấy kết quả từ hàm 1
        $combatDamage = $this->getTotalDamage(); 
        
        // Công thức chỉ số phụ
        $hp = $this->base_hp;
