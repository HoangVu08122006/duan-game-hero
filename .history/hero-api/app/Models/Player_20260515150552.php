...
protected static function booted()
{
    ...
    static::created(function ($player) {
        ...
        foreach ($allWeapons as $index => $weapon) {
            \App\Models\PlayerWeapon::create([
                ...
                'weapon_id' => $weapon->id,
                'level'     => 1,
                // LV 1: Tặng món đầu tiên (index 0)
                'is_owned'    => ($index === 0), 
                'is_equipped' => ($index === 0),
            ]);
        }
    ...
    static::saving(function ($player) {
        // Logic lên cấp (giữ nguyên của bạn)
        // ...
        
        // Kiểm tra mở khóa theo mốc 100 level
        $player->checkAndUnlockByLevelMilestone();
    });
}
...
public function checkAndUnlockByLevelMilestone() {
    // Lấy danh sách vũ khí sắp xếp theo ID hoặc độ mạnh
    $weapons = \App\Models\Weapon::orderBy('id', 'asc')->get();

    foreach ($weapons as $index => $weapon) {
        // Công thức: Món thứ 1 (index 0) -> LV 1
        // Món thứ 2 (index 1) -> LV 100, Món thứ 3 -> LV 200...
        $requiredLv = ($index === 0) ? 1 : ($index * 100);

        if ($this->level >= $requiredLv) {
            $pw = \App\Models\PlayerWeapon::where('player_id', $this->id)
                                         ->where('weapon_id', $weapon->id)
                                         ->first();
            if ($pw && !$pw->is_owned) {
                $pw->update(['is_owned' => true]);
            }
        }
    }
}
