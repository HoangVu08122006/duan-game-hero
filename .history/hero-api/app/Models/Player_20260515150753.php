...
public function checkAndUnlockWeaponByMilestone() {
    // Lấy danh sách vũ khí trong hòm đồ của Player này
    $playerWeapons = $this->weapons()->with('weapon')->get();

    foreach ($playerWeapons as $index => $pw) {
        // Món 1 (index 0): LV 1
        // Món 2 (index 1): LV 100
        // Món 3 (index 2): LV 200...
        $requiredLv = ($index === 0) ? 1 : ($index * 100);

        // Nếu đủ cấp và chưa sở hữu thì mở khóa
        if ($this->level >= $requiredLv && !$pw->is_owned) {
            $pw->is_owned = true;
            $pw->save();
        }
    }
