...
{
    // 1. Lấy Damage từ hàm getTotalDamage() đã có sẵn (đã tính chuẩn 1 weapon + 1 pet)
    $combatDamage = $this->getTotalDamage(); 

    // 2. Tính các chỉ số bổ trợ
    $hp = $this->base_hp;
    $critRate = 10 + ($this->upgraded_crit_rate_lv * 2);
    $critDamage = 100 + ($this->upgraded_crit_damage_lv * 5);
    $speed = $this->upgraded_speed_lv * 0.2;

    // 3. Tổng hợp lực chiến
    // Lưu ý: $combatDamage đã bao gồm Base ATK + Weapon + Pet
    // Nên công thức chỉ cần cộng thêm các phần bổ trợ
    $total = $combatDamage 
             + ($hp / 10) 
             + ($critRate * 50) 
             + ($critDamage * 20) 
    ...
    /**
 * Công thức tính lực chiến tập trung (Dùng chung cho cả User và Admin)
 */
public function calculateTotalPower($player) 
{
    // 1. Sát thương tấn công & HP (Lấy từ giá trị hiện tại đã nâng cấp trong DB)
    $attack = $player->base_attack; 
    $hp = $player->base_hp;
    
    // 2. Các chỉ số phụ
    $critRate = 10 + ($player->upgraded_crit_rate_lv * 2);      // Gốc 10% + 2% mỗi cấp
    $critDamage = 100 + ($player->upgraded_crit_damage_lv * 5); // Gốc 100% + 5% mỗi cấp
    $speed = $player->upgraded_speed_lv * 0.2;                  // 0.2 mỗi cấp

    // 3. Tính toán tổng lực chiến theo công thức bạn đã định nghĩa
    $total = $attack 
             + ($hp / 10) 
             + ($critRate * 50) 
             + ($critDamage * 20) 
