...
public function updatePlayerStats(Request $request, $id)
{
    ...
    // 1. CẬP NHẬT TẤT CẢ TỪ REQUEST (Đảm bảo tên trường trong Request trùng với tên ô input)
    $player->upgraded_hp_lv          = $request->input('upgraded_hp_lv', $player->upgraded_hp_lv);
    $player->upgraded_attack_lv      = $request->input('upgraded_attack_lv', $player->upgraded_attack_lv);
    $player->upgraded_speed_lv       = $request->input('upgraded_speed_lv', $player->upgraded_speed_lv);
    $player->upgraded_crit_rate_lv   = $request->input('upgraded_crit_rate_lv', $player->upgraded_crit_rate_lv);
    $player->upgraded_crit_damage_lv = $request->input('upgraded_crit_damage_lv', $player->upgraded_crit_damage_lv);

    // 2. Tính lại chỉ số gốc (vẫn dùng công thức 100/1000 cũ)
    $player->base_attack = (int)round(100 * pow(1.08, $player->upgraded_attack_lv));
    $player->base_hp     = (int)round(1000 * pow(1.12, $player->upgraded_hp_lv));

    // 3. QUAN TRỌNG: Lấy lực chiến từ Vũ khí và Pet hiện tại
    // Bạn cần cộng thêm cái này vào hàm calculateTotalPower hoặc cộng trực tiếp tại đây
    $weaponPower = $player->weapons()->where('is_equipped', true)->sum('current_damage');
    
    // 4. Tính toán tổng lực chiến
    $player->total_power = $this->calculateTotalPower($player) + $weaponPower; 
