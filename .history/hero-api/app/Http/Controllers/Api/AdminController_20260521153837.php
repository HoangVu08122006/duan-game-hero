...
public function getPlayerTotalPowerForAdmin($id)
{
    try {
        ...
        $totalPower = $totalPower + $weaponDamage + $petDamage;
...
public function getPlayerTotalPowerForAdmin($id)
{
    try {
        ...
        return response()->json([
            ...
            'details' => [
                'base_damage' => $player->current_attack,
                'weapon_damage' => $player->weapons()->where('is_equipped', true)->sum('damage'),
                'pet_power' => $player->pet_skills['total_power'] ?? 0,
            ]
        ], 200);

    } catch (\Exception $e) {
        // Ghi log để bạn vào storage/logs/laravel.log xem chi tiết
        \Log::error("Lỗi API admin: " . $e->getMessage());
        
        return response()->json([
            'status' => 'error', 
            'message' => 'Lỗi hệ thống khi tính lực chiến.',
