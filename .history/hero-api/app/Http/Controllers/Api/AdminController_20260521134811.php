...
public function getTotalPower(Request $request)
{
    // 1. Lấy thông tin người chơi đang đăng nhập
    $player = $request->user();
    
    // Mặc định lấy sát thương cơ bản của Hero làm gốc
    $heroDamage = $player->base_attack ?? 50; 

    // 2. Tính Damage của VŨ KHÍ đang trang bị (is_equipped = 1)
    $weaponDamage = 0;
    ...
    if ($equippedWeapon) {
        // Sử dụng Accessor current_damage có sẵn trong Model PlayerWeapon của bạn
        $weaponDamage = $equippedWeapon->current_damage ?? 0;
    }

    // 3. Tính Damage của PET đang trang bị (is_equipped = 1)
    $petDamage = 0;
    ...
    if ($equippedPet) {
        ...
        // Áp dụng công thức tăng 10% mỗi cấp giống hàm getMyPets của bạn
        $mainPetDame = $petBaseAtk * pow(1.1, $petLv - 1);

        // Tính thêm sát thương các Skill nếu pet đủ cấp độ
        $skill1 = 0;
        ...
        // Tổng damage của riêng con Pet này
    ...
    // 5. Cập nhật lại cột total_power trong bảng người chơi để lưu lại giá trị mới nhất
    $player->update([
        'total_power' => (int)round($totalPower)
    ]);

    // 6. Trả kết quả về cho Unity
    return response()->json([
        'success' => true,
        'message' => 'Tính toán tổng lực chiến thành công!',
        'player_name' => $player->name,
        'details' => [
            'hero_base_damage' => (int)$heroDamage,
            'equipped_weapon_damage' => (int)round($weaponDamage),
            'equipped_pet_damage' => (int)round($petDamage),
        ],
        'total_power' => (int)round($totalPower)
    ], 200);
}
