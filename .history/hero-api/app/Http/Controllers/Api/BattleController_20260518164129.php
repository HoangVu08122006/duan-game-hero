...
class BattleController extends Controller
{
     ...
     */
    /**
 * API: Người chơi chủ động bấm nút để leo lại lên tầng cao sau khi bị lùi
 */
public function retryNextFloor(Request $request)
{
    $player = $request->user();

    // Tiến lên tầng tiếp theo theo lệnh thủ công của người chơi
    $player->current_floor += 1;
    $player->kill_count = 0; // Đặt lại về 0 để bắt đầu tính 5 con ở tầng cao
    $player->current_hp = $player->base_hp; // Bơm đầy máu để tự tin phục thù
    $player->save();

    return response()->json([
        'success' => true,
        'message' => "Bắt đầu thử thách lại tầng " . $player->current_floor,
        'player_status' => [
