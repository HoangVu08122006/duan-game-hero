...
class RewardController extends Controller
{
    ...
    return DB::transaction(function () use ($playerId) {
        ...
        if (!$config) {
            return response()->json(['message' => 'Không tìm thấy cấu hình phần thưởng'], 404);
        }

        // CỘNG QUÀ: Dùng 'reward_type' thay vì 'type'
        // Kiểm tra xem là 'gold' hay 'exp' để tăng đúng cột trong bảng 'players'
        if ($config->reward_type === 'gold') {
            $player->increment('gold', $config->amount);
        } elseif ($config->reward_type === 'exp') {
            $player->increment('exp', $config->amount);
        }
