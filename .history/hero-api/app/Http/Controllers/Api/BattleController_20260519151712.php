...
class BattleController extends Controller
{
    ...
    /**
     * API 3: Lấy trạng thái hiển thị DPS và tiến trình (Đã tối ưu công thức tính Skill Boss)
     */
    public function getBattleStatus(Request $request)
    {
        $player = $request->user();
        $floor = max(1, $player->current_floor);
        $heroDps = max(1, $player->getTotalDamage()); // Sát thương Hero gây ra mỗi giây
        ...
        if ($isBossFloor) {
            ...
            // Tính toán lượng sát thương trung bình Boss gây ra cho Hero mỗi giây (gồm đánh thường + skill)
            // Giả sử tốc độ đánh thường của Boss là 1 giây/phát, và cứ 3 giây kích hoạt thêm skill thay thế hoặc cộng dồn
            ...
            $bossDpsValue = $normalAtk; // Quái thường chỉ đánh thường
        }

        // Thời gian ước tính Hero hạ gục đối phương
        ...
        return response()->json([
            ...
            'monster_stats' => [
                ...
                'max_hp' => $monsterHp,
                'atk' => $normalAtk,
                'avg_dps_received' => round($bossDpsValue, 2) // Sát thương trung bình Hero phải nhận mỗi giây
            ],
            'battle_config' => [
                'floor' => $floor,
