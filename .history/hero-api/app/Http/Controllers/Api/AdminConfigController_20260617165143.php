...
class AdminConfigController extends Controller
{
    ...
    public function store(Request $request)
{
    $request->validate(['name' => 'required', 'duration' => 'required', 'items' => 'required']);
    ...
    return DB::transaction(function () use ($request) {
        if ($request->has('id')) {
            ...
            $config->items()->delete(); // Xóa cũ lưu mới cho đơn giản
        } else {
            // Create
            $config = RewardConfig::create(['name' => $request->name, 'duration' => $request->duration, 'status' => 'draft']);
        }

        foreach ($request->items as $item) {
            $config->items()->create($item);
        }
