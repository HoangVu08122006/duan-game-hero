...
{
    // 1. Validate toàn bộ dữ liệu có thể sửa
    ...
    // 2. Cập nhật dữ liệu từ mảng đã validate
    $player->fill($validated);

    // 3. Tính toán lại các chỉ số phụ (Lực chiến)
    // Lưu ý: Logic tính lực chiến nên đưa vào một hàm trong Model (ví dụ: $player->recalculatePower())
    // để code Controller sạch hơn nữa.
    ...
    // Validate dữ liệu như cũ...
        $validated = $request->validate([ /* ... */ ]);
        $player = Player::findOrFail($id);

        // --- ĐOẠN NÀY DÙNG ĐỂ GHI LOG ---
        foreach ($validated as $field => $newValue) {
            // Chỉ lưu log nếu giá trị thực sự thay đổi
            if ($player->{$field} != $newValue) {
    ...
    return response()->json([
        ...
        'player' => $player->refresh() // refresh() để lấy dữ liệu mới nhất sau khi Model xử lý saving()
    ]);
}
