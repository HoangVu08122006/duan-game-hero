...
public function store_weapon(Request $request) {
    try {
        ...
        $validatedData = $request->validate([
            ...
            'image_weapon' => 'nullable|image|mimes:jpeg,png,jpg,gif|max:2048',
        ]);

        // 2. Xử lý lưu file trước khi vào Transaction
        ...
        return DB::transaction(function () use ($data) {
            // Dùng $data thay vì $validatedData (vì $data đã có image_weapon)
            ...
            return response()->json(['message' => 'Thành công', 'weapon' => $weapon], 201);
        });

    } catch (\Exception $e) {
        return response()->json(['message' => 'Có lỗi', 'error' => $e->getMessage()], 500);
    }
}
