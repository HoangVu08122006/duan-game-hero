...
public function getPlayerTotalPowerForAdmin($id)
{
    try {
        ...
        $heroDamage = $player->current_attack ?? 50; 
...
public function updatePlayerStats(Request $request, $id)
{
    ...
    // 1. Cập nhật các cấp độ từ Request
    ...
    // 2. Định nghĩa chỉ số gốc
    $baseAtk = 100; // Gốc Level 0
    $baseHp  = 1000; // Gốc Level 0

    // 3. Tính hệ số dồn (Multiplier) từ các chỉ số phụ
    // Gốc: CR 10% (0.1), CD 100% (1.0), Tốc độ 1s (1 đòn/s)
    $critRate   = 0.10 + ($player->upgraded_crit_rate_lv * 0.02);   // +2% mỗi cấp
    $critDamage = 1.00 + ($player->upgraded_crit_damage_lv * 0.05); // +5% mỗi cấp
    $speedBonus = 1.00 + ($player->upgraded_speed_lv * 0.05);       // +5% mỗi cấp (đánh nhanh hơn)

    // Công thức tính Sát thương thực tế (dồn vào base_attack)
    // = (Gốc * Lũy thừa Level) * Hệ số Chí mạng * Hệ số Tốc độ
    $multiplier = pow(1.08, $player->upgraded_attack_lv) 
                * (1 + ($critRate * ($critDamage - 1))) 
                * $speedBonus;

    $player->base_attack = (int)round($baseAtk * $multiplier);
    $player->base_hp     = (int)round($baseHp * pow(1.12, $player->upgraded_hp_lv));

    // 4. Lấy Damage từ Vũ khí và Pet (Sửa lỗi 42S22)
    // Lấy qua quan hệ 'weapon' để không bị lỗi không tìm thấy cột
    $weaponAtk = $player->weapons()->where('is_equipped', true)->with('weapon')->get()->sum(function($w) {
        return $w->weapon->damage ?? 0; // Đảm bảo bảng 'weapons' của bạn có cột 'damage'
    });

    // Lấy Damage từ Pet
    $petAtk = $player->playerPets()->where('is_equipped', true)->with('pet')->get()->sum(function($p) {
        return $p->pet->base_attack ?? 0; // Đảm bảo bảng 'pets' của bạn có cột 'base_attack'
    });

    // 5. Tổng lực chiến = base_attack + Weapon + Pet
    $player->total_power = $player->base_attack + $weaponAtk + $petAtk;
