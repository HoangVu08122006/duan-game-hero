...
public function updatePlayerStats(Request $request, $id)
{
    ...
    // Cập nhật các cấp độ từ request
    ...
    // Tính lại Base Stats dựa trên level mới (Thuật toán giống Client)
    // Giả sử base gốc ban đầu (ở level 0) là hằng số, 
    // hoặc bạn cần lưu base_initial để tính toán chuẩn xác.
    // Dưới đây là cách tính theo hệ số bạn cung cấp:
    $player->base_attack = (int)round($player->base_attack * pow(1.08, $player->upgraded_attack_lv));
    $player->base_hp = (int)round($player->base_attack * pow(1.12, $player->upgraded_hp_lv));

    // Cập nhật lực chiến tổng
    $player->total_power = $this->calculateTotalPower($player);
    $player->save();

    return response()->json(['status' => 'success', 'message' => 'Đã cập nhật chỉ số thành công']);
}
}