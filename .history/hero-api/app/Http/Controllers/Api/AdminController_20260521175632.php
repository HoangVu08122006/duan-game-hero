...
public function updatePlayerStats(Request $request, $id)
{
    ...
    // 1. Cập nhật Level từ request của Admin
    ...
    // 2. Tính lại chỉ số Tấn công và HP dựa trên level mới
    // LƯU Ý: Ở đây mình giả định 100 và 1000 là chỉ số gốc (Level 0)
    // Nếu bạn có cột base_initial thì thay bằng $player->base_initial_attack
    $base_attack_initial = 100; 
    $base_hp_initial = 1000;

    // Áp dụng công thức tăng trưởng (tăng 8% mỗi cấp cho ATK, 12% cho HP)
    $player->base_attack = (int)round($base_attack_initial * pow(1.08, $player->upgraded_attack_lv));
    $player->base_hp = (int)round($base_hp_initial * pow(1.12, $player->upgraded_hp_lv));

    // 3. Tính lại lực chiến (Dùng chính hàm bạn đã viết ở HeroController)
    // Bạn nên đưa hàm calculateTotalPower vào Model Player hoặc một Service để dùng chung
    $player->total_power = $this->calculateTotalPower($player);
    ...
    return response()->json([
        'status' => 'success',
        'message' => 'Đã cập nhật chỉ số nhân vật thành công!',
        'new_stats' => [
