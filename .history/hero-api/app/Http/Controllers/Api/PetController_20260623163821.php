...
{
    $request->validate([
        'player_pet_id' => 'required|integer', // ID của dòng trong bảng player_pets
    ]);

    $player = $request->user();
    $newPetId = $request->player_pet_id;

    // 1. Kiểm tra xem pet mới có thuộc về người chơi không
    $newPet = $player->playerPets()->where('id', $newPetId)->first();

    if (!$newPet) {
        return response()->json(['message' => 'Pet không tồn tại hoặc không thuộc sở hữu của bạn!'], 404);
    }

    // 2. Sử dụng Database Transaction để đảm bảo tính toàn vẹn
    DB::transaction(function () use ($player, $newPet) {
        // Tắt tất cả pet hiện tại
        $player->playerPets()->update(['is_equipped' => 0]);
        
        // Bật pet mới
        $newPet->update(['is_equipped' => 1]);
    });

    return response()->json([
        'success' => true,
        'message' => 'Đã chuyển sang pet mới thành công!'
    ]);
}
