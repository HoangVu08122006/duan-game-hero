...
public function adminSetStats(Request $request, $id)
{
    try {
        ...
        // TỰ ĐỘNG CẬP NHẬT LẠI CHỈ SỐ GỐC & LỰC CHIẾN
        $this->recalculatePlayerStats($player); 
// Đảm bảo $player sau khi recalc đã có base_attack, total_power mới nhất

return response()->json([
    'status' => 'success',
    'message' => 'Cập nhật thành công!',
    'new_stats' => [
        'base_attack' => $player->base_attack, // Giá trị mới sau khi tính toán
        'base_hp'     => $player->base_hp,     // Giá trị mới
        'total_power' => $player->total_power  // Giá trị mới
    ]
]);
