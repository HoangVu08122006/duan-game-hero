...
public function store_weapon(Request $request) {
    try {
        $validatedData = $request->validate([
            ...
            'base_attack' => 'required|numeric',
            'required_hero_level' => 'required|numeric',
        ]);
        // 1. Validate
    $request->validate([
        'image_weapon' => 'nullable|image|mimes:jpeg,png,jpg,gif|max:2048', // Giới hạn file ảnh 2MB
    ]);

    $data = $request->except('image_weapon');

    // 2. Xử lý lưu file
    if ($request->hasFile('image_weapon')) {
        $path = $request->file('image_weapon')->store('weapons', 'public');
        $data['image_weapon'] = '/storage/' . $path; // Lưu đường dẫn này vào DB
    }

        // Sử dụng Transaction: Nếu thêm vũ khí lỗi, hệ thống sẽ rollback, không thêm vào player_weapons
        DB::transaction(function () use ($validatedData) {
            // 1. Tạo vũ khí
            $weapon = Weapon::create($validatedData);

            // 2. Lấy tất cả người chơi
            $players = Player::all();

            // 3. Tự động thêm vũ khí này cho tất cả người chơi
            foreach ($players as $player) {
                PlayerWeapon::create([
                    'player_id' => $player->id,
                    'weapon_id' => $weapon->id,
                    'level' => 1, // Mặc định level 1
                    'is_equipped' => 0, // Mặc định chưa trang bị
                ]);
            }
        ...
        return response()->json(['message' => 'Vũ khí đã được thêm cho tất cả người chơi!'], 201);

    } catch (\Exception $e) {
        Log::error('Lỗi khi thêm vũ khí và gán cho người chơi: ' . $e->getMessage());
