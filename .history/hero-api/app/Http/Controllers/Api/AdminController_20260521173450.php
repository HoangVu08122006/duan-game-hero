...
public function updatePlayerStats(Request $request, $id)
{
    ...
    // 1. Gán giá trị gốc (Bạn nên lưu cái này trong DB khi user mới tạo tài khoản)
    // Nếu chưa có cột, hãy thay bằng con số cố định khởi điểm của game
    $base_attack_initial = 100; 
    $base_hp_initial = 1000;

    // 2. Cập nhật cấp độ từ Request
    ...
    // 3. Tăng trực tiếp vào base_attack/base_hp bằng cách nhân lũy thừa từ gốc
    // Cách này đảm bảo: Tăng level -> Chỉ số tăng. Giảm level -> Chỉ số giảm về đúng cấp đó.
    ...
    // 4. Tính lại lực chiến (hàm này đã bao gồm cả Crit, Speed rồi)
    $player->total_power = $this->calculateTotalPower($player);
    $player->save();
