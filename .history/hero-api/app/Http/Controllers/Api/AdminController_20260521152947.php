...
public function getPlayerTotalPowerForAdmin($id)
{
    ...
    try {
        // Tính toán toàn bộ thông qua Model
        $totalPower = $player->calculateTotalPower();
        
        // Cập nhật vào DB
        $player->update(['total_power' => $totalPower]);

        // Trả về dữ liệu chi tiết cho FE
        return response()->json([
            'status' => 'success',
            'message' => 'Cập nhật lực chiến thành công!',
            'calculated_total_power' => $totalPower,
            'damage_details' => [
                'hero_base' => $player->getTotalDamage(),
                'equipped_weapon' => $player->weapons()->where('is_equipped', true)->sum('current_damage'),
                'equipped_pet' => $player->pet_skills['total_power'] ?? 0,
            ]
        ], 200);

    } catch (\Exception $e) {
        // Ghi log lỗi để dễ debug
        Log::error("Lỗi tính lực chiến Player ID $id: " . $e->getMessage());
        
        return response()->json([
            'status' => 'error',
            'message' => 'Đã xảy ra lỗi hệ thống khi tính lực chiến.',
            'error' => $e->getMessage()
        ], 500);
    }
}
