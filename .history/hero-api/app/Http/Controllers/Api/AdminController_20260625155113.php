...
use App\Events\WeaponUpdatedEvent;
...
public function login(Request $request)
{
    ...
    // Tìm admin theo 'name' vì bảng của bạn cột tên là 'name'
    $admin = Admin::where('name', $request->username)->first();

    // Kiểm tra admin tồn tại và mật khẩu khớp
    if (!$admin || !Hash::check($request->password, $admin->password)) {
        return response()->json(['message' => 'Sai tài khoản hoặc mật khẩu!'], 401);
    }

    $token = $admin->createToken('admin_token')->plainTextToken;
    return response()->json(['token' => $token, 'message' => 'Đăng nhập thành công!']);
}
    ...
    public function getPlayerTotalPowerForAdmin($id)
    {
        try {
            ...
            $heroDamage = ($stats['atk'])
                + ($stats['hp'] / 10)
                + ($stats['crit_r'] / 100 * 2000)
                + (($stats['crit_d'] / 100 - 1) * 1000)
                + ($stats['speed'] * 100);
    ...
    // Danh sách
    public function index_weapon()
    {
        return response()->json(Weapon::all());
    }

    // Xem chi tiết
    public function show_weapon($id)
    {
        return response()->json(Weapon::findOrFail($id));
    }

    public function store_weapon(Request $request)
    {
        try {
            $data = $request->only(['name', 'base_attack', 'required_hero_level']);

            if ($request->hasFile('image_weapon')) {
                $path = $request->file('image_weapon')->store('weapons', 'public');
                $data['image_weapon'] = '/storage/' . $path;
            }

            return DB::transaction(function () use ($data) {
                $weapon = Weapon::create($data);

                // Logic gán cho người chơi
                $players = Player::all();
                foreach ($players as $player) {
                    PlayerWeapon::create(['player_id' => $player->id, 'weapon_id' => $weapon->id, 'level' => 1, 'is_equipped' => 0]);
                }

                broadcast(new WeaponUpdatedEvent($weapon));
    ...
    // Trong WeaponController.php

    public function update_weapon(Request $request, $id)
    {
        try {
            ...
            // Xử lý file
            if ($request->hasFile('image_weapon')) {
                // Xóa ảnh cũ nếu cần thiết (tùy chọn)
                // Storage::disk('public')->delete(str_replace('/storage/', '', $weapon->image_weapon));
    ...
    // Xóa
    public function destroy_weapon($id)
    {
        Weapon::destroy($id);
...
public function updatePlayerStats(Request $request, $id)
{
    $validated = $request->validate([
        ...
        'upgraded_hp_lv' => 'sometimes|integer|min:0',
        'upgraded_attack_lv' => 'sometimes|integer|min:0',
        'upgraded_speed_lv' => 'sometimes|integer|min:0',
        'upgraded_crit_rate_lv' => 'sometimes|integer|min:0',
        'upgraded_crit_damage_lv' => 'sometimes|integer|min:0',
    ]);

    $player = Player::findOrFail($id);

    // --- ĐOẠN NÀY PHẢI ĐẶT TRƯỚC KHI GỌI $player->fill() ---
    foreach ($validated as $field => $newValue) {
        // Lấy giá trị hiện tại TRƯỚC KHI cập nhật
    ...
    // 2. Bây giờ mới cập nhật dữ liệu
    $player->fill($validated);

    // 3. Tính toán lại các chỉ số phụ...
    ...
    return response()->json([
        'status' => 'success',
        'message' => 'Cập nhật và ghi log thành công!',
...
{
    // Lấy dữ liệu kèm theo thông tin Player
    $logs = \App\Models\AccountCreationLog::with('player')->latest()->get();

    // Định dạng lại dữ liệu để FE hiển thị dễ hơn (tùy chọn)
    $formattedLogs = $logs->map(function ($log, $index) {
        return [
            'stt' => $index + 1, // STT theo thứ tự hiển thị
            'player_name' => $log->player ? $log->player->name : 'N/A', // Lấy tên từ bảng players
            'created_at' => $log->created_at->format('Y-m-d H:i:s'),
            'creation_method' => $log->creation_method === 'manual' ? 'Admin tạo' : 'Người chơi tự tạo'
        ];
    });
...
public function getLogsUpdate($id)
{
    // Lấy 20 dòng log gần nhất của người chơi này
    $logs = \App\Models\PlayerLog::where('player_id', $id)
...
// API Khóa tài khoản
    public function ban(Request $request, $id)
    {
        ...
        if (!$player) {
            return response()->json(['message' => 'Người chơi không tồn tại'], 404);
        }

        // Cập nhật trạng thái
        DB::table('players')->where('id', $id)->update(['is_banned' => 1]);
        ...
        DB::table('player_logs')->insert([
            ...
            'old_value'   => '0',
            'new_value'   => '1',
            'admin_name'  => $request->user()->name ?? 'System', // Lấy tên admin nếu đã đăng nhập
            'created_at'  => now(),
        ]);
    ...
    public function unban(Request $request, $id)
    {
        ...
        // Cập nhật trạng thái
        DB::table('players')->where('id', $id)->update(['is_banned' => 0]);
        ...
        DB::table('player_logs')->insert([
            ...
            'old_value'   => '1',
            'new_value'   => '0',
            'admin_name'  => $request->user()->name ?? 'System',
            'created_at'  => now(),
        ]);

        return response()->json(['message' => 'Đã gỡ khóa tài khoản thành công']);
    }
}
