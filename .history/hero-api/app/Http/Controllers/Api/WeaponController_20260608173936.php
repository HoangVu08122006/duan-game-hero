...
{
    // Admin truyền lên: player_id, player_weapon_id (id của bảng player_weapons), và level_moi
    $playerId = $request->input('player_id');
    $pwId = $request->input('id'); 
    $newLevel = $request->input('level');

    $playerWeapon = \App\Models\PlayerWeapon::where('player_id', $playerId)
        ->where('id', $pwId)
        ->with('weapon')
        ->firstOrFail();
    ...
    DB::transaction(function () use ($playerWeapon, $oldLevel, $newLevel, $playerId) {
        // Cập nhật cấp độ trực tiếp
        $playerWeapon->update(['level' => $newLevel]);

        // GHI LOG ADMIN THAO TÁC
        \App\Models\PlayerLog::create([
            'player_id'  => $playerId,
    ...
    return response()->json([
        'success' => true,
        'message' => 'Admin đã cập nhật cấp độ vũ khí thành công!'
    ]);
}
