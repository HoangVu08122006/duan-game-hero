...
public function adminSetStats(Request $request, $id)
{
    try {
        ...
        // 1. Cập nhật các chỉ số gốc trực tiếp từ request của Admin
        // Nếu Admin truyền lên giá trị mới, ta ghi đè vào DB
        if ($request->has('base_attack')) {
            $player->base_attack = $request->base_attack;
        }
        if ($request->has('base_hp')) {
            $player->base_hp = $request->base_hp;
        }
        
        // Cập nhật các level nâng cấp (để vẫn giữ được tiến độ nâng cấp nếu cần)
        $player->fill($request->only([
            'upgraded_attack_lv', 'upgraded_hp_lv', 'upgraded_crit_rate_lv', 
            'upgraded_speed_lv', 'upgraded_crit_damage_lv'
        ]));

        // 2. Tính lại tổng lực chiến dựa trên các chỉ số mới vừa cập nhật
        $player->total_power = $this->calculateTotalPower($player);
        $player->save();

        return response()->json([
            'status' => 'success',
            'message' => 'Cập nhật chỉ số trực tiếp thành công!',
            'new_stats' => [
