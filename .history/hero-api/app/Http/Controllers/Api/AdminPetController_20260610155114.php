...
class AdminPetController extends Controller
{
    public function adminUpdatePetLevel(Request $request)
    {
        ...
        try {
            // SỬA CHỖ NÀY: Dùng pet_id thay vì id
    $pet = PlayerPet::where('pet_id', $validated['player_pet_id']) 
                    ->where('player_id', $validated['player_id'])
                    ->first();

    if (!$pet) {
        return response()->json(['message' => 'Lỗi: Không tìm thấy thú cưng của người chơi này!'], 404);
    }
