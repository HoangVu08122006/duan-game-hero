   ...
   public function equip(Request $request)
{
    ...
    $weaponId = $request->input('id'); 

    $player = Player::findOrFail($playerId);
    
    // 1. Tìm vũ khí mục tiêu TRƯỚC
    $targetWeapon = $player->weapons()->with('weapon')->where('id', $weaponId)->first();

    // 2. Kiểm tra dữ liệu và điều kiện TRƯỚC KHI update
    if (!$targetWeapon) {
    ...
    // 3. Sử dụng Transaction để đảm bảo tính toàn vẹn (tất cả hoặc không gì cả)
    DB::transaction(function () use ($player, $targetWeapon) {
        // Lấy vũ khí cũ để ghi log
        $oldWeapon = $player->weapons()->where('is_equipped', 1)->with('weapon')->first();
        
        // Tắt trang bị cũ (của tất cả vũ khí thuộc player)
        $player->weapons()->update(['is_equipped' => 0]);
        
        // Bật trang bị cho vũ khí mới
...
public function upgrade(Request $request)
{
    ...
    // --- GIỮ NGUYÊN: KIỂM TRA ĐIỀU KIỆN MỞ KHÓA ---
    ...
    // Lấy cấp cũ để ghi log
    $oldLevel = $playerWeapon->level;

    // --- THỰC HIỆN NÂNG CẤP VÀ GHI LOG TRONG TRANSACTION ---
    DB::transaction(function () use ($player, $playerWeapon, $cost, $oldLevel) {
        // Trừ vàng và tăng cấp
        $player->decrement('gold', $cost);
        $playerWeapon->increment('level');

        // THÊM LOGIC MỚI: Ghi log lịch sử nâng cấp
        \App\Models\PlayerLog::create([
            'player_id'  => $player->id,
            'field_name' => "Nâng cấp " . $playerWeapon->weapon->name,
            'old_value'  => "Cấp " . $oldLevel,
            'new_value'  => "Cấp " . ($oldLevel + 1),
            'created_at' => now(),
        ]);
    });
...
public function adminUpdateWeaponLevel(Request $request)
{
    $playerId = $request->input('player_id');
    $pwId = $request->input('id'); // Phải khớp với key 'id' gửi từ axios
    $newLevel = $request->input('level');

    // 1. Tìm bản ghi chính xác
    $playerWeapon = \App\Models\PlayerWeapon::where('id', $pwId)
        ->where('player_id', $playerId)
        ->with('weapon')
        ->first();

    if (!$playerWeapon) {
        return response()->json(['message' => 'Không tìm thấy vũ khí!'], 404);
    }

    // 2. Lấy cấp độ cũ TRƯỚC khi thực hiện thay đổi
    $oldLevel = $playerWeapon->level;

    // 3. Update
    DB::transaction(function () use ($playerWeapon, $newLevel, $playerId, $oldLevel) {
        $playerWeapon->update(['level' => $newLevel]);

        \App\Models\PlayerLog::create([
            'player_id'  => $playerId,
            'field_name' => "Admin chỉnh cấp " . $playerWeapon->weapon->name,
            'old_value'  => "Cấp " . $oldLevel, // Đã lấy được cấp cũ
