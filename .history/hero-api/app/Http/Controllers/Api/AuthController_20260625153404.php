        ...
        $data = DB::transaction(function () use ($request) {
            ...
            $player = Player::firstOrCreate(
                ...
                [
                    ...
                    'base_hp' => 1000,
                    'base_attack' => 100,
                    'base_crit_rate' => 20.00,
                    'base_crit_damage' => 50.00,
                    'base_speed' => 1,
            ...
            // --- ĐOẠN CODE GHI LOG MỚI ---
            // Kiểm tra xem ai là người tạo
            $isAdmin = Auth::guard('admin')->check();
            \App\Models\AccountCreationLog::create([
                'player_id' => $player->id,
                'created_by_admin_id' => $isAdmin ? auth('admin')->id() : null, // Luôn an toàn với null
                'creation_method' => $isAdmin ? 'manual' : 'player'
            ]);
...
// --- GHI LOG ĐĂNG NHẬP ---
    \App\Models\PlayerLog::create([
        'player_id'  => $player->id,
        'field_name' => "Hệ thống",
        'old_value'  => "Ngoại tuyến",
        'new_value'  => "Đã đăng nhập vào hệ thống",
        'created_at' => now(),
    ]);
    return response()->json([
        'token' => $token,
...
// API ĐĂNG XUẤT
    public function logout(Request $request)
    {
        try {
            ...
            \App\Models\PlayerLog::create([
                'player_id'  => $player->id,
                'field_name' => "Hệ thống",
                'old_value'  => "Đang trực tuyến",
                'new_value'  => "Đã đăng xuất",
                'created_at' => now(),
            ]);
    ...
    \App\Models\PlayerLog::create([
        'player_id'  => $player->id,
        'field_name' => "Hệ thống",
        'old_value'  => "Đang hoạt động trên nhiều thiết bị",
        'new_value'  => "Đăng xuất khỏi tất cả thiết bị",
        'created_at' => now(),
    ]);
