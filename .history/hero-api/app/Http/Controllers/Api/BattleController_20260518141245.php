    ...
    public function spawn(Request $request)
{
    ...
    // Đảm bảo tầng hiện tại tối thiểu là 1, tránh lỗi dữ liệu player bị bằng 0 hoặc âm
    ...
    if ($isBossFloor) {
        ...
        // Fallback: Nếu không có boss nào được thiết kế riêng, lấy đại một con quái thường làm Boss tạm thời
        if (!$boss) {
            $boss = Monster::inRandomOrder()->first();
        }
        
        // Nếu database trống rỗng hoàn toàn (không có cả boss lẫn quái)
        if (!$boss) {
            return response()->json([
                'success' => false, 
                'message' => 'Database hoàn toàn không có dữ liệu Boss hoặc Quái'
            ], 404);
        }
        ...
        // Tìm quái có min_floor nhỏ hơn hoặc bằng tầng hiện tại
        $monster = Monster::where('min_floor', '<=', $floor)->inRandomOrder()->first();
        
        // Fallback: Nếu min_floor cấu hình sai dẫn đến không tìm thấy quái, 
        // tự động lấy con quái có min_floor thấp nhất trong database để thế vào.
        if (!$monster) {
            $monster = Monster::orderBy('min_floor', 'asc')->first();
        }
        
        // Nếu database trống rỗng hoàn toàn (không có con quái nào)
        if (!$monster) {
            return response()->json([
                'success' => false, 
