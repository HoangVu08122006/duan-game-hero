...
{
    try {
        $player = Player::with(['weapons.weapon', 'playerPets.pet'])->find($id);
        ...
        // 1. Hero Damage
        $heroDamage = (float)($player->base_attack ?? 50);

        // 2. Weapon Damage
        $equippedWeapon = $player->weapons->where('is_equipped', true)->first();
        $weaponDamage = $equippedWeapon ? (float)($equippedWeapon->current_damage ?? 0) : 0;

        // 3. Pet Damage
        $petDamage = 0;
        $equippedPet = $player->playerPets->where('is_equipped', true)->first();

        if ($equippedPet && $equippedPet->pet) {
            $petLv = (int)($equippedPet->level ?? 1);
            $petBaseAtk = (float)($equippedPet->pet->base_attack ?? 50);
            ...
            $skill1 = ($petLv >= 30) ? $baseSkill1 * pow(1.1, $petLv - 30) : 0;
        ...
        // Cập nhật với kiểm tra lỗi
        $player->update(['total_power' => (int)round($totalPower)]);
        ...
        return response()->json([
            ...
            'damage_details' => [
                ...
                'pet' => (int)$petDamage,
                'total' => (int)round($totalPower)
            ]
        ], 200);

    } catch (\Exception $e) {
        \Log::error("Error calculating power: " . $e->getMessage());
        return response()->json(['status' => 'error', 'message' => 'Lỗi tính toán: ' . $e->getMessage()], 500);
    }
}
