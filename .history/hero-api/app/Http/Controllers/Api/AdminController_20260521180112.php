...
public function updatePlayerStats(Request $request, $id)
{
    ...
    // 2. TÍNH TOÁN BASE_ATTACK DỒN CHỈ SỐ (Logic cốt lõi)
    // Giá trị gốc (ví dụ 100)
    $base = 100;
    
    // Tính công thức: Gốc * tăng trưởng Level * Bonus chỉ số phụ
    // Bạn có thể tùy chỉnh hệ số nhân ở đây (ví dụ: mỗi level Crit Rate tăng thêm 2% dame)
    $multiplier = pow(1.08, $player->upgraded_attack_lv) 
                * (1 + ($player->upgraded_crit_rate_lv * 0.02)) 
                * (1 + ($player->upgraded_crit_damage_lv * 0.01))
                * (1 + ($player->upgraded_speed_lv * 0.02));

    $player->base_attack = (int)round($base * $multiplier);
    $player->base_hp     = (int)round(1000 * pow(1.12, $player->upgraded_hp_lv));

    // 3. Tính Total Power = base_attack + pet + weapon (Lấy dữ liệu an toàn)
    $weaponAtk = $player->weapons()->where('is_equipped', true)->with('weapon')->get()->sum(function($w) {
        return $w->weapon->damage ?? 0; // Thay 'damage' bằng tên cột đúng trong bảng 'weapons' của bạn
    });
    
    $petAtk = $player->playerPets()->where('is_equipped', true)->with('pet')->get()->sum(function($p) {
        return $p->pet->base_attack ?? 0; // Thay 'base_attack' bằng tên cột đúng trong bảng 'pets'
    });
